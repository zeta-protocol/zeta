...
import (
	...
	eventsv1 "github.com/zeta-protocol/zeta/protos/zeta/events/v1"
...
func TestSqlBrokerSubscriberCallbacks(t *testing.T) {
	s1 := testSQLBrokerSubscriber{
		...
		zetaTimeCh: make(chan time.Time),
	...
	// BlockEnd event should cause a flush of subscribers, followed by commit and then an update to subscribers zetatime,
	...
	assert.Equal(t, time.Unix(0, beginEvent.BeginBlock().Timestamp), <-s1.zetaTimeCh)
	...
	expectedBlock := entities.Block{
		ZetaTime: time.Unix(0, beginEvent.BeginBlock().Timestamp).Truncate(time.Microsecond),
		Hash:     hash,
	...
	assert.Equal(t, time.Unix(0, beginEvent.BeginBlock().Timestamp).Truncate(time.Microsecond), <-s1.zetaTimeCh)
	assert.Equal(t, true, <-transactionManager.withTransactionCalls)
...
func TestSqlBrokerTimeEventSentToAllSubscribers(t *testing.T) {
	...
	assert.Equal(t, time.Unix(0, blockEvent.BeginBlock().Timestamp).
		Truncate(time.Microsecond), <-s1.zetaTimeCh)
	assert.Equal(t, time.Unix(0, blockEvent.BeginBlock().Timestamp).
		Truncate(time.Microsecond), <-s2.zetaTimeCh)
}
...
func newTestSQLBrokerSubscriber(eventType events.Type) *testSQLBrokerSubscriber {
	return &testSQLBrokerSubscriber{
		...
		zetaTimeCh: make(chan time.Time, 100),
	}
}
...
func newBlockEventSource() *blockEventSource {
	return &blockEventSource{
		zetaTime:    time.Now().Truncate(time.Millisecond),
		blockHeight: 1,
	}
}
