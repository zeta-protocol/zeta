...
type OrderEventWithZetaTime struct {
	events.Order
	zetaTime time.Time
}
...
func (oe *OrderEventWithZetaTime) GetOrder() *zeta.Order {
...
func (b *Broker) startSending(t events.Type, evt events.Event) {
	...
	if t == events.TimeUpdate {
		...
		b.zetaTime = timeUpdate.Time().Truncate(time.Microsecond)
	}

	if t == events.OrderEvent {
		orderEvent := evt.(*events.Order)
		evt = &OrderEventWithZetaTime{*orderEvent, b.zetaTime}
	}
