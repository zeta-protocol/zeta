...
func handleCandleConnectionRequest(ctx context.Context, client TradingDataServiceClientV2, market *zeta.Market, sinceRaw string, toRaw *string,
	interval zeta.Interval, pagination *v2.Pagination,
) (*v2.CandleDataConnection, error) {
	since, err := zetatime.Parse(sinceRaw)
	if err != nil {
		return nil, err
	}
...
func toV2IntervalString(interval zeta.Interval) (string, error) {
	switch interval {
	case zeta.Interval_INTERVAL_BLOCK:
		return "block", nil
	case zeta.Interval_INTERVAL_I1M:
		return "1 minute", nil
	case zeta.Interval_INTERVAL_I5M:
		return "5 minutes", nil
	case zeta.Interval_INTERVAL_I15M:
		return "15 minutes", nil
	case zeta.Interval_INTERVAL_I1H:
		return "1 hour", nil
	case zeta.Interval_INTERVAL_I6H:
		return "6 hours", nil
	case zeta.Interval_INTERVAL_I1D:
		return "1 day", nil
	default:
...
func handleProposalsRequest(ctx context.Context, client TradingDataServiceClientV2, party *types.Party, ref *string, inType *v2.ListGovernanceDataRequest_Type,
	inState *zeta.Proposal_State, pagination *v2.Pagination,
