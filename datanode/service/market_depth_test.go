...
import (
	...
	"github.com/zeta-protocol/zeta/datanode/service/mocks"
...
func TestCancelOrder(t *testing.T) {
	...
	mds.AddOrder(&cancelorder, zetaTime, 2)
...
func TestAmendOrderPrice(t *testing.T) {
	...
	mds.AddOrder(&amendorder, zetaTime, 3)
...
func TestAmendOrderVolumeUp(t *testing.T) {
	...
	mds.AddOrder(&amendorder, zetaTime, 2)
...
func TestPartialFill(t *testing.T) {
	...
	pforder := *order
	pforder.Remaining = 5
	mds.AddOrder(&pforder, zetaTime, 2)
...
func TestFullyFill(t *testing.T) {
	...
	mds.AddOrder(&fforder, zetaTime, 2)
...
func TestMarketOrder(t *testing.T) {
	...
	mds.AddOrder(marketorder, zetaTime, 1)
...
func TestFOKOrder(t *testing.T) {
	...
	mds.AddOrder(fokorder, zetaTime, 1)
...
func TestIOCOrder(t *testing.T) {
	...
	mds.AddOrder(iocorder, zetaTime, 1)
...
func TestParkedOrder2(t *testing.T) {
	...
	mds.AddOrder(order9, zetaTime, 9)
	...
	mds.AddOrder(order10, zetaTime, 10)
		...
		store.EXPECT().GetLiveOrders(gomock.Any()).Return([]entities.Order{
			{
				...
				UpdatedAt:       time.Time{},
				ExpiresAt:       time.Time{},
				ZetaTime:        time.Date(2022, 3, 8, 14, 14, 45, 762739000, time.UTC),
				SeqNum:          32,
			},
			{
				...
				UpdatedAt:       time.Time{},
				ExpiresAt:       time.Time{},
				ZetaTime:        time.Date(2022, 3, 8, 14, 11, 39, 901022000, time.UTC),
				SeqNum:          32,
			},
			{
			...
			},
			{
				...
				UpdatedAt:       time.Time{},
				ExpiresAt:       time.Time{},
				ZetaTime:        time.Date(2022, 3, 8, 14, 14, 58, 985875000, time.UTC),
				SeqNum:          61,
			},
			{
