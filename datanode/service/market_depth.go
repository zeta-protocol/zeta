...
func (m *MarketDepth) Initialise(ctx context.Context) error {
	...
	for _, liveOrder := range liveOrders {
		...
		m.AddOrder(order, liveOrder.ZetaTime, liveOrder.SeqNum)
	}
...
func (m *MarketDepth) AddOrder(order *types.Order, zetaTime time.Time, sequenceNumber uint64) {
	...
	// we truncate the zetaTime by microsecond because Postgres only supports microsecond
	...
	seqNum := uint64(zetaTime.Truncate(time.Microsecond).UnixNano()) + sequenceNumber
