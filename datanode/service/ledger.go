...
type Ledger struct {
	store             ledgerStore
	transferResponses []*zeta.LedgerMovement
	observer          utils.Observer[*zeta.LedgerMovement]
}

func NewLedger(store ledgerStore, log *logging.Logger) *Ledger {
	return &Ledger{
		store:    store,
		observer: utils.NewObserver[*zeta.LedgerMovement]("ledger", log, 0, 0),
	}
}
...
func (l *Ledger) Flush(ctx context.Context) error {
	...
	l.transferResponses = []*zeta.LedgerMovement{}
	return nil
}
...
func (l *Ledger) Observe(ctx context.Context, retries int) (<-chan []*zeta.LedgerMovement, uint64) {
	ch, ref := l.observer.Observe(ctx,
		retries,
		func(tr *zeta.LedgerMovement) bool {
			return true
		})
	return ch, ref
}
