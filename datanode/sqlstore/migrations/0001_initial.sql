...
create table blocks
(
    zeta_time     TIMESTAMP WITH TIME ZONE NOT NULL PRIMARY KEY,
    height        BIGINT                   NOT NULL,
    hash          BYTEA                    NOT NULL
);
select create_hypertable('blocks', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
(
    ...
    zeta_time           TIMESTAMP WITH TIME ZONE NOT NULL,
    PRIMARY KEY (id, zeta_time)
);

CREATE VIEW assets_current AS (
  SELECT DISTINCT ON (id) * FROM assets ORDER BY id, zeta_time DESC
);
...
(
    ...
    tx_hash   BYTEA NOT NULL,
    zeta_time TIMESTAMP WITH TIME ZONE
);
...
(
    ...
    PRIMARY KEY(zeta_time, account_id) INCLUDE (balance)
);

select create_hypertable('balances', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
create index on balances (account_id, zeta_time) INCLUDE(balance);
...
CREATE INDEX ON ledger (account_from_id, zeta_time DESC);
CREATE INDEX ON ledger (account_to_id, zeta_time DESC);
CREATE INDEX ON ledger (type, zeta_time DESC);
...
CREATE TABLE orders (
    ...
    PRIMARY KEY(zeta_time, seq_num)
);

SELECT create_hypertable('orders', 'zeta_time', chunk_time_interval => INTERVAL '1 day');

create index on orders (id, zeta_time desc, seq_num desc);
create index on orders (created_at desc, id, zeta_time desc, seq_num desc);
create index on orders (market_id, created_at desc, id, zeta_time desc, seq_num desc);
create index on orders (party_id, created_at desc, id, zeta_time desc, seq_num desc);
create index on orders (reference, created_at desc, id, zeta_time desc, seq_num desc);
...
CREATE INDEX ON orders_live (market_id, zeta_time DESC);
CREATE INDEX ON orders_live (party_id, zeta_time DESC);
CREATE INDEX ON orders_live (reference, zeta_time DESC);
...
BEGIN
    ...
    -- As per https://github.com/zetaprotocol/specs-internal/blob/master/protocol/0024-OSTA-order_status.md
...
-- As per https://github.com/zetaprotocol/zeta/issues/8149, only LIMIT type (1) orders with status active (1) and parked (8)
-- and time_in_force != IOC (3) and time_in_force != FOK (4) are considered live.
    ...
    THEN
        ...
        VALUES(new.id, new.market_id, new.party_id, new.side, new.price,
               ...
               new.tx_hash, new.zeta_time, new.seq_num, new.post_only, new.reduce_only);
    END IF;
...
CREATE OR REPLACE VIEW orders_current_versions AS (
    SELECT DISTINCT ON (id, version) * FROM orders ORDER BY id, version DESC, zeta_time DESC
);
...
FROM orders
ORDER BY orders.created_at DESC, orders.id, orders.zeta_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_market
AS
SELECT DISTINCT ON (orders.created_at, orders.market_id, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.market_id, orders.id, orders.zeta_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_party
AS
SELECT DISTINCT ON (orders.created_at, orders.party_id, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.party_id, orders.id, orders.zeta_time DESC, orders.seq_num DESC;

CREATE OR REPLACE VIEW orders_current_desc_by_reference
AS
SELECT DISTINCT ON (orders.created_at, orders.reference, orders.id) *
FROM orders
ORDER BY orders.created_at DESC, orders.reference, orders.id, orders.zeta_time DESC, orders.seq_num DESC;
...
SELECT market_id,  zeta_time as period_start,
       first(price, synthetic_time) AS open,
       last(price, synthetic_time) AS close,
       max(price) AS high,
       min(price) AS low,
...
create index on market_data (market, zeta_time);
...
$$
BEGIN
    INSERT INTO current_market_data(synthetic_time,tx_hash,zeta_time,seq_num,market,mark_price,best_bid_price,best_bid_volume,
    ...
    VALUES(NEW.synthetic_time, NEW.tx_hash, NEW.zeta_time, NEW.seq_num, NEW.market,
    ...
    ON CONFLICT(market) DO UPDATE SET
                                      synthetic_time=EXCLUDED.synthetic_time,tx_hash=EXCLUDED.tx_hash,zeta_time=EXCLUDED.zeta_time,seq_num=EXCLUDED.seq_num,market=EXCLUDED.market,mark_price=EXCLUDED.mark_price,
...
CREATE TABLE IF NOT EXISTS nodes_announced (
  ...
  tx_hash               BYTEA NOT NULL,
  zeta_time             TIMESTAMP WITH TIME ZONE NOT NULL,
  PRIMARY KEY(node_id, epoch_seq, zeta_time)
);
...
SELECT create_hypertable('rewards', 'zeta_time', chunk_time_interval => INTERVAL '1 day', migrate_data => true);
...
select create_hypertable('delegations', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
$$
BEGIN
    INSERT INTO delegations_current(party_id,node_id,epoch_id,amount,tx_hash,zeta_time,seq_num)
    VALUES(NEW.party_id,NEW.node_id,NEW.epoch_id,NEW.amount,NEW.tx_hash,NEW.zeta_time,NEW.seq_num)
...
SELECT create_hypertable('markets', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
create table if not exists deposits (
    ...
    tx_hash  bytea not null,
    zeta_time timestamp with time zone not null,
    primary key (id, party_id, zeta_time)
);
CREATE INDEX ON deposits(party_id);

select create_hypertable('deposits', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE VIEW deposits_current AS (
    ...
    SELECT DISTINCT ON (id, party_id) * FROM deposits ORDER BY id, party_id, zeta_time DESC
);
...
select create_hypertable('withdrawals', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE VIEW withdrawals_current AS (
    ...
    SELECT DISTINCT ON (id, party_id) * FROM withdrawals ORDER BY id, party_id, zeta_time DESC
);
...
CREATE TABLE votes(
  ...
  PRIMARY KEY (proposal_id, party_id, zeta_time)
);

CREATE INDEX ON votes(party_id);

CREATE VIEW votes_current AS (
  SELECT DISTINCT ON (proposal_id, party_id) * FROM votes ORDER BY proposal_id, party_id, zeta_time DESC
);
...
create table if not exists margin_levels (
    ...
    tx_hash BYTEA NOT NULL,
    zeta_time timestamp with time zone not null,
    PRIMARY KEY(zeta_time, account_id)
);

select create_hypertable('margin_levels', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
INSERT INTO current_margin_levels(account_id,
                                  ...
                                  tx_hash,
                                  zeta_time) VALUES(NEW.account_id,
                                                    NEW.timestamp,
...
CREATE MATERIALIZED VIEW conflated_margin_levels
            WITH (timescaledb.continuous, timescaledb.materialized_only = true) AS
SELECT account_id, time_bucket('1 minute', zeta_time) AS bucket,
       last(maintenance_margin, zeta_time) AS maintenance_margin,
       last(search_level, zeta_time) AS search_level,
       last(initial_margin, zeta_time) AS initial_margin,
...
create table if not exists risk_factors (
    ...
    tx_hash bytea not null,
    zeta_time timestamp with time zone not null,
    primary key (market_id, zeta_time)
);

CREATE VIEW risk_factors_current AS (
    SELECT DISTINCT ON (market_id) * FROM risk_factors ORDER BY market_id, zeta_time DESC
);
...
SELECT create_hypertable('checkpoints', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
CREATE TABLE positions(
  ...
  primary key (zeta_time, party_id, market_id)
);

select create_hypertable('positions', 'zeta_time', chunk_time_interval => INTERVAL '1 day');

CREATE INDEX ON positions(party_id, market_id, zeta_time);

CREATE MATERIALIZED VIEW conflated_positions
            WITH (timescaledb.continuous, timescaledb.materialized_only = true) AS
SELECT market_id, party_id, time_bucket('1 hour', zeta_time) AS bucket,
 last(open_volume, zeta_time) AS open_volume,
 last(realised_pnl, zeta_time) AS realised_pnl,
 last(unrealised_pnl, zeta_time) AS unrealised_pnl,
 last(average_entry_price, zeta_time) AS average_entry_price,
 last(average_entry_market_price, zeta_time) AS average_entry_market_price,
...
$$
BEGIN
    INSERT INTO positions_current(market_id,party_id,open_volume,realised_pnl,unrealised_pnl,average_entry_price,average_entry_market_price,loss,adjustment,tx_hash,zeta_time,pending_open_volume,pending_realised_pnl,pending_unrealised_pnl,pending_average_entry_price,pending_average_entry_market_price, loss_socialisation_amount, distressed_status)
    VALUES(NEW.market_id,NEW.party_id,NEW.open_volume,NEW.realised_pnl,NEW.unrealised_pnl,NEW.average_entry_price,NEW.average_entry_market_price,NEW.loss,NEW.adjustment,NEW.tx_hash,NEW.zeta_time,NEW.pending_open_volume,NEW.pending_realised_pnl,NEW.pending_unrealised_pnl,NEW.pending_average_entry_price,NEW.pending_average_entry_market_price, NEW.loss_socialisation_amount, NEW.distressed_status)
...
$$
BEGIN
    INSERT INTO oracle_data_current(signers,data,matched_spec_ids,broadcast_at,tx_hash,zeta_time,seq_num)
    VALUES(NEW.signers,NEW.data,NEW.matched_spec_ids,NEW.broadcast_at,NEW.tx_hash,NEW.zeta_time,NEW.seq_num)
...
select create_hypertable('liquidity_provisions', 'zeta_time', chunk_time_interval => INTERVAL '1 day');
...
-- Active (1), Undeployed (5), Pending (6)
    ...
    THEN
        INSERT INTO live_liquidity_provisions(id, party_id, created_at, updated_at,
                                              market_id, commitment_amount, fee, sells, buys, version, status, reference, tx_hash, zeta_time)
        VALUES(NEW.id, NEW.party_id, NEW.created_at, NEW.updated_at,
               NEW.market_id, NEW.commitment_amount, NEW.fee, NEW.sells,
               NEW.buys, NEW.version, NEW.status, NEW.reference, NEW.tx_hash, NEW.zeta_time);
    END IF;
...
CREATE VIEW transfers_current AS ( SELECT DISTINCT ON (id, from_account_id, to_account_id) * FROM transfers ORDER BY id, from_account_id, to_account_id, zeta_time DESC);
...
$$
BEGIN
    INSERT INTO stake_linking_current (id, stake_linking_type, ethereum_timestamp, party_id, amount, stake_linking_status, finalized_at, foreign_tx_hash, foreign_block_height, foreign_block_time, log_index, ethereum_address, tx_hash, zeta_time)
    VALUES (NEW.id,
...
CREATE TABLE IF NOT EXISTS protocol_upgrade_proposals(
    ...
    zeta_release_tag TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS core_snapshots(
    ...
    zeta_core_version TEXT null,
    tx_hash bytea not null,
    zeta_time TIMESTAMP WITH TIME ZONE NOT NULL,
    PRIMARY KEY(zeta_time, block_height)
);
...
DROP INDEX IF EXISTS positions_party_id_market_id_zeta_time_idx;
