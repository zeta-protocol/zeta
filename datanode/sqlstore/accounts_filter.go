...
func filterAccountsQuery(af entities.AccountFilter, includeZetaTime bool) (string, []interface{}, error) {
	var args []interface{}
	var err error

	query := `SELECT id, party_id, asset_id, market_id, type, tx_hash FROM ACCOUNTS `
	if includeZetaTime {
		query = `SELECT id, party_id, asset_id, market_id, type, tx_hash, zeta_time FROM ACCOUNTS `
	}
...
func currentAccountBalancesQuery() string {
	return `SELECT ACCOUNTS.id, ACCOUNTS.party_id, ACCOUNTS.asset_id, ACCOUNTS.market_id, ACCOUNTS.type,
			current_balances.balance, current_balances.tx_hash, current_balances.zeta_time
			FROM ACCOUNTS JOIN current_balances ON ACCOUNTS.id = current_balances.account_id `
}

func accountBalancesQuery() string {
	return `SELECT ACCOUNTS.id, ACCOUNTS.party_id, ACCOUNTS.asset_id, ACCOUNTS.market_id, ACCOUNTS.type,
			balances.balance, balances.tx_hash, balances.zeta_time
