...
const (
	sqlMarketsColumns = `id, tx_hash, zeta_time, instrument_id, tradable_instrument, decimal_places,
...
set
	...
	if _, err := m.Connection.Exec(ctx, query, market.ID, market.TxHash, market.ZetaTime, market.InstrumentID, market.TradableInstrument, market.DecimalPlaces,
