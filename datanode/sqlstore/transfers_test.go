...
func testTransfersGetTransferToOrFromParty(t *testing.T) {
	...
	transfer, err := entities.TransferFromProto(ctx, sourceTransferProto, generateTxHash(), block.ZetaTime, accounts)
	...
	transfer, err = entities.TransferFromProto(ctx, sourceTransferProto2, generateTxHash(), block.ZetaTime, accounts)
	assert.NoError(t, err)
	err = transfers.Upsert(ctx, transfer)
	assert.NoError(t, err)
...
func testTransfersGetTransfersByParty(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto, generateTxHash(), block.ZetaTime, accounts)
	...
	transfer, _ = entities.TransferFromProto(ctx, sourceTransferProto2, generateTxHash(), block.ZetaTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func testTransfersGetFromAccountAndGetToAccount(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto1, generateTxHash(), block.ZetaTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func testTransfersUpdatesInDifferentBlocks(t *testing.T) {
	...
	deliverOn := block.ZetaTime.Add(1 * time.Hour)
	...
	block = getTestBlock(t, ctx, block.ZetaTime.Add(1*time.Microsecond))
...
func testGetByTxHash(t *testing.T) {
	...
	transfer, _ := entities.TransferFromProto(ctx, sourceTransferProto, txHash, block.ZetaTime, accounts)
	transfers.Upsert(ctx, transfer)
...
func getTestBlock(t *testing.T, ctx context.Context, testTime time.Time) entities.Block {
	...
	zetaTime := time.UnixMicro(testTime.UnixMicro())
	block := addTestBlockForTime(t, ctx, blocks, zetaTime)
	return block
}
...
func addTransfers(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, transferStore *sqlstore.Transfers) []entities.Transfer {
	...
	block := addTestBlockForTime(t, ctx, bs, zetaTime)
	accounts := sqlstore.NewAccounts(connectionSource)
	accountFrom, accountTo := getTestAccounts(t, ctx, accounts, block)
