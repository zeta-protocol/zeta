...
func testInsertMarginLevels(t *testing.T) {
	...
	marginLevel, err := entities.MarginLevelsFromProto(ctx, marginLevelProto, accountStore, generateTxHash(), block.ZetaTime)
...
func getMarginLevelProto() *zeta.MarginLevels {
	return getMarginLevelWithMaintenanceProto("1000", "deadbeef", "deadbeef", time.Now().UnixNano())
}

func getMarginLevelWithMaintenanceProto(maintenanceMargin, partyID, marketID string, timestamp int64) *zeta.MarginLevels {
...
func testGetMarginLevelsByPartyID(t *testing.T) {
	...
	marginLevel1, err := entities.MarginLevelsFromProto(ctx, ml1, accountStore, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")

	marginLevel2, err := entities.MarginLevelsFromProto(ctx, ml2, accountStore, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")
	...
	block = blockSource.getNextBlock(t, ctx)
	marginLevel3, err := entities.MarginLevelsFromProto(ctx, ml3, accountStore, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")

	marginLevel4, err := entities.MarginLevelsFromProto(ctx, ml4, accountStore, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting margin levels proto to database entity")
...
func populateMarginLevelPaginationTestData(t *testing.T, ctx context.Context) (*sqlstore.MarginLevels, map[int]entities.Block, map[int]entities.MarginLevels) {
	...
	for i, ml := range margins {
		block := blockSource.getNextBlock(t, ctx)
		mlProto := getMarginLevelWithMaintenanceProto(ml.maintenanceMargin, ml.partyID, ml.marketID, block.ZetaTime.UnixNano())
		mlEntity, err := entities.MarginLevelsFromProto(ctx, mlProto, accountStore, generateTxHash(), block.ZetaTime)
		require.NoError(t, err, "Converting margin levels proto to database entity")
