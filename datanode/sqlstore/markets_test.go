...
func getByTxHashReturnsMatchingMarkets(t *testing.T) {
	...
	assert.Equal(t, market.ZetaTime, foundMarkets[0].ZetaTime)
...
func shouldInsertAValidMarketRecord(t *testing.T) {
	...
	market, err := entities.NewMarketFromProto(marketProto, generateTxHash(), block.ZetaTime)
...
func shouldUpdateAValidMarketRecord(t *testing.T) {
	...
	t.Run("should insert a valid market record to the database", func(t *testing.T) {
		...
		err = pgxscan.Get(ctx, conn, &got, `select * from markets where id = $1 and zeta_time = $2`, market.ID, market.ZetaTime)
		assert.NoError(t, err)
	...
	t.Run("should add the updated market record to the database if the block number has changed", func(t *testing.T) {
		...
		market, err := entities.NewMarketFromProto(newMarketProto, generateTxHash(), newBlock.ZetaTime)
		require.NoError(t, err, "Converting market proto to database entity")
		...
		err = pgxscan.Get(ctx, conn, &gotFirstBlock, `select * from markets where id = $1 and zeta_time = $2`, market.ID, block.ZetaTime)
		assert.NoError(t, err)
		assert.Equal(t, "TEST_INSTRUMENT", market.InstrumentID)

		assert.Equal(t, marketProto.TradableInstrument, gotFirstBlock.TradableInstrument.ToProto())

		err = pgxscan.Get(ctx, conn, &gotSecondBlock, `select * from markets where id = $1 and zeta_time = $2`, market.ID, newBlock.ZetaTime)
		assert.NoError(t, err)
		assert.Equal(t, "TEST_INSTRUMENT", market.InstrumentID)
...
func getTestMarket() *zeta.Market {
	return &zeta.Market{
		Id: "DEADBEEF",
		TradableInstrument: &zeta.TradableInstrument{
			Instrument: &zeta.Instrument{
				Id:   "TEST_INSTRUMENT",
				Code: "TEST",
				Name: "Test Instrument",
				Metadata: &zeta.InstrumentMetadata{
					Tags: []string{"AAA", "BBB"},
				},
				Product: &zeta.Instrument_Future{
					Future: &zeta.Future{
						SettlementAsset: "Test Asset",
						QuoteName:       "Test Quote",
						DataSourceSpecForSettlementData: &zeta.DataSourceSpec{
							Id:        "",
							CreatedAt: 0,
							UpdatedAt: 0,
							Data: zeta.NewDataSourceDefinition(
								zeta.DataSourceDefinitionTypeExt,
							).SetOracleConfig(
								&zeta.DataSourceSpecConfiguration{
									Signers: nil,
									Filters: nil,
								},
							),
						...
						DataSourceSpecForTradingTermination: &zeta.DataSourceSpec{
							Id:        "",
							CreatedAt: 0,
							UpdatedAt: 0,
							Data: zeta.NewDataSourceDefinition(
			...
			MarginCalculator: &zeta.MarginCalculator{
			...
			RiskModel: &zeta.TradableInstrument_SimpleRiskModel{
				SimpleRiskModel: &zeta.SimpleRiskModel{
					Params: &zeta.SimpleModelParams{
						FactorLong:           0,
						FactorShort:          0,
						MaxMoveUp:            0,
						MinMoveDown:          0,
		...
		PriceMonitoringSettings: &zeta.PriceMonitoringSettings{
		...
		TradingMode: zeta.Market_TRADING_MODE_CONTINUOUS,
		State:       zeta.Market_STATE_ACTIVE,
		MarketTimestamps: &zeta.MarketTimestamps{
			Proposed: 0,
			Pending:  0,
			Open:     0,
			Close:    0,
...
func testCursorPaginationReturnsAllMarkets(t *testing.T) {
	...
	wantStartCursor := entities.NewCursor(
		entities.MarketCursor{
			ZetaTime: blockTimes["02a16077"],
			ID:       "02a16077",
		}.String(),
	).Encode()
	wantEndCursor := entities.NewCursor(
		entities.MarketCursor{
			ZetaTime: blockTimes["fb1528a5"],
			ID:       "fb1528a5",
		}.String(),
	).Encode()
	assert.Equal(t, entities.PageInfo{
...
func testCursorPaginationReturnsPageTraversingForward(t *testing.T) {
	...
	wantStartCursor := entities.NewCursor(
		entities.MarketCursor{
			ZetaTime: blockTimes["7a797e0e"],
			ID:       "7a797e0e",
		}.String(),
	).Encode()
	wantEndCursor := entities.NewCursor(
		entities.MarketCursor{
			ZetaTime: blockTimes["b7c84b8e"],
			ID:       "b7c84b8e",
		}.String(),
	).Encode()
	assert.Equal(t, entities.PageInfo{
