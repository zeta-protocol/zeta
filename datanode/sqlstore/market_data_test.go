...
func shouldInsertAValidMarketDataRecord(t *testing.T) {
	...
	err = md.Add(&entities.MarketData{
		...
		PriceMonitoringBounds: []*zeta.PriceMonitoringBounds{
			{
				MinValidPrice: "1",
				MaxValidPrice: "2",
				Trigger: &zeta.PriceMonitoringTrigger{
					Horizon:          100,
					Probability:      "0.5",
...
func getLatestMarketData(t *testing.T) {
	...
	want := entities.MarketData{
		...
		LiquidityProviderFeeShares: []*zeta.LiquidityProviderFeeShare{
			{
		...
		ZetaTime: time.Date(2022, 2, 11, 10, 5, 41, 0, time.UTC),
	}
...
func setupMarketData(t *testing.T, ctx context.Context) (*sqlstore.MarketData, error) {
	...
	for {
		...
		if _, alreadyAdded := addedBlocksAt[marketData.ZetaTime.UnixNano()]; !alreadyAdded {
			...
			require.NoError(t, err)
			addedBlocksAt[marketData.ZetaTime.UnixNano()] = struct{}{}
		}
...
func mustParsePriceMonitoringBounds(t *testing.T, value string) []*zeta.PriceMonitoringBounds {
	t.Helper()
	if strings.ToLower(value) == "null" {
		return nil
	}

	var bounds []*zeta.PriceMonitoringBounds
...
func mustParseLiquidity(t *testing.T, value string) []*zeta.LiquidityProviderFeeShare {
	t.Helper()
	if strings.ToLower(value) == "null" {
		return nil
	}

	var liquidity []*zeta.LiquidityProviderFeeShare
...
func csvToMarketData(t *testing.T, line []string, seqNum int) *entities.MarketData {
	...
	zetaTime := mustParseTimestamp(t, line[csvColumnZetaTime])
	syntheticTime := zetaTime.Add(time.Duration(seqNum) * time.Microsecond)
