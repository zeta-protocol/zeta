...
func testAddDepositForNewBlock(t *testing.T) {
	...
	deposit, err := entities.DepositFromProto(depositProto, generateTxHash(), block.ZetaTime)
...
func testUpdateDepositForBlockIfExists(t *testing.T) {
	...
	deposit.Status = entities.DepositStatus(zeta.Deposit_STATUS_FINALIZED)
	...
	err = pgxscan.Get(ctx, conn, &status, `select status from deposits where id = $1 and zeta_time = $2`, deposit.ID, deposit.ZetaTime)
	assert.NoError(t, err)
...
func testInsertDepositUpdatesIfNewBlock(t *testing.T) {
	...
	block = addTestBlock(t, ctx, bs)
	depositProto.Status = zeta.Deposit_STATUS_FINALIZED
	deposit, err = entities.DepositFromProto(depositProto, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting market proto to database entity")
...
func testDepositsGetByParty(t *testing.T) {
	...
	deposit, err := entities.DepositFromProto(depositProto1, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	depositProto1.Status = zeta.Deposit_STATUS_FINALIZED
	deposit, err = entities.DepositFromProto(depositProto1, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	deposit, err = entities.DepositFromProto(depositProto2, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting market proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	deposit, err = entities.DepositFromProto(depositProto2, generateTxHash(), block.ZetaTime)
	depositProto2.Status = zeta.Deposit_STATUS_FINALIZED
	require.NoError(t, err, "Converting market proto to database entity")
...
func getTestDeposit(id, party, asset, amount, txHash string, ts int64) *zeta.Deposit {
	return &zeta.Deposit{
		Id:                id,
		Status:            zeta.Deposit_STATUS_OPEN,
		PartyId:           party,
		Asset:             asset,
		Amount:            amount,
		TxHash:            txHash,
...
func addDeposits(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, ds *sqlstore.Deposits) []entities.Deposit {
	...
	for i := 0; i < 10; i++ {
		...
		depositProto := getTestDeposit(fmt.Sprintf("deadbeef%02d", i+1), testID, testID,
			strconv.FormatInt(amount, 10), helpers.GenerateID(), zetaTime.UnixNano())
		deposit, err := entities.DepositFromProto(depositProto, generateTxHash(), zetaTime)
...
func testDepositsPaginationBetweenDatesNoPagination(t *testing.T) {
	...
	startDate := testDeposits[3].ZetaTime
	endDate := testDeposits[8].ZetaTime
...
func testDepositsPaginationBetweenDatesFirst(t *testing.T) {
	...
	startDate := testDeposits[2].ZetaTime
	endDate := testDeposits[8].ZetaTime
