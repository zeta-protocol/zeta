...
func testUpdateMarketRiskFactors(t *testing.T) {
	...
	time.Sleep(5 * time.Microsecond) // Ensure we get a different zeta time
...
func testAddRiskFactor(t *testing.T) {
	...
	riskFactor, err := entities.RiskFactorFromProto(riskFactorProto, generateTxHash(), block.ZetaTime)
...
func getRiskFactorProto() *zeta.RiskFactor {
