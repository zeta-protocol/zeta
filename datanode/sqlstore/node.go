...
func selectNodeQuery() string {
	...
	this_epoch AS (
		SELECT nodes.id AS node_id,
			COALESCE(SUM(current_delegations.amount) FILTER (WHERE current_delegations.party_id = nodes.zeta_pub_key), 0) AS staked_by_operator,
			COALESCE(SUM(current_delegations.amount) FILTER (WHERE current_delegations.party_id != nodes.zeta_pub_key), 0) AS staked_by_delegates,
...
func (store *Node) AddNodeAnnouncedEvent(ctx context.Context, nodeID string, zetatime time.Time, aux *entities.ValidatorUpdateAux) error {
	...
	_, err := store.Connection.Exec(ctx, `
		...
		VALUES
			($1, $2, $3, $4, $5)
		ON CONFLICT (node_id, epoch_seq, zeta_time) DO UPDATE SET
...
func (store *Node) UpdatePublicKey(ctx context.Context, kr *entities.KeyRotation) error {
	...
	_, err := store.Connection.Exec(ctx, `UPDATE nodes SET zeta_pub_key = $1 WHERE id = $2`, kr.NewPubKey, kr.NodeID)
...
func (store *Node) GetByTxHash(ctx context.Context, txHash entities.TxHash) ([]entities.NodeBasic, error) {
	...
	query := `SELECT id, zeta_pub_key, tendermint_pub_key, ethereum_address, name, location, 
