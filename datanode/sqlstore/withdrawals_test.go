...
func testAddWithdrawalForNewBlock(t *testing.T) {
	...
	withdrawalProto := getTestWithdrawal(testID, testID, testID, testAmount, testID, block.ZetaTime)

	withdrawal, err := entities.WithdrawalFromProto(withdrawalProto, generateTxHash(), block.ZetaTime)
...
func testUpdateWithdrawalForBlockIfExists(t *testing.T) {
	...
	withdrawal.Status = entities.WithdrawalStatus(zeta.Withdrawal_STATUS_FINALIZED)
	...
	err = pgxscan.Get(ctx, conn, &status, `select status from withdrawals where id = $1 and zeta_time = $2`, withdrawal.ID, withdrawal.ZetaTime)
	assert.NoError(t, err)
...
func testInsertWithdrawalUpdatesIfNewBlock(t *testing.T) {
	...
	block = addTestBlock(t, ctx, bs)
	withdrawalProto.Status = zeta.Withdrawal_STATUS_FINALIZED
	withdrawal, err = entities.WithdrawalFromProto(withdrawalProto, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting withdrawal proto to database entity")
...
func testWithdrawalsGetByParty(t *testing.T) {
	...
	block := addTestBlock(t, ctx, bs)
	withdrawalProto1 := getTestWithdrawal(testID, testID, testID, testAmount, testID, block.ZetaTime)
	withdrawalProto1.Id = "deadbeef01"

	withdrawalProto2 := getTestWithdrawal(testID, testID, testID, testAmount, testID, block.ZetaTime)
	withdrawalProto2.Id = "deadbeef02"

	want := make([]entities.Withdrawal, 0)

	withdrawal, err := entities.WithdrawalFromProto(withdrawalProto1, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting withdrawal proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	withdrawalProto1.Status = zeta.Withdrawal_STATUS_FINALIZED
	withdrawal, err = entities.WithdrawalFromProto(withdrawalProto1, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting withdrawal proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	withdrawal, err = entities.WithdrawalFromProto(withdrawalProto2, generateTxHash(), block.ZetaTime)
	require.NoError(t, err, "Converting withdrawal proto to database entity")
	...
	block = addTestBlock(t, ctx, bs)
	withdrawal, err = entities.WithdrawalFromProto(withdrawalProto2, generateTxHash(), block.ZetaTime)
	withdrawalProto2.Status = zeta.Withdrawal_STATUS_FINALIZED
	require.NoError(t, err, "Converting withdrawal proto to database entity")
...
func getTestWithdrawal(id, party, asset, amount, txHash string, ts time.Time) *zeta.Withdrawal {
	return &zeta.Withdrawal{
		Id:                 id,
		PartyId:            party,
		Amount:             amount,
		Asset:              asset,
		Status:             zeta.Withdrawal_STATUS_OPEN,
		Ref:                "deadbeef",
		TxHash:             txHash,
		CreatedTimestamp:   ts.UnixNano(),
		WithdrawnTimestamp: ts.UnixNano(),
		Ext: &zeta.WithdrawExt{
			Ext: &zeta.WithdrawExt_Erc20{
				Erc20: &zeta.Erc20WithdrawExt{
...
func addWithdrawals(ctx context.Context, t *testing.T, bs *sqlstore.Blocks, ws *sqlstore.Withdrawals) []entities.Withdrawal {
	...
	for i := 0; i < 10; i++ {
		...
		withdrawalProto := getTestWithdrawal(fmt.Sprintf("deadbeef%02d", i+1), testID, testID,
			strconv.FormatInt(amount, 10), helpers.GenerateID(), zetaTime)
		withdrawal, err := entities.WithdrawalFromProto(withdrawalProto, generateTxHash(), zetaTime)
		require.NoError(t, err, "Converting withdrawal proto to database entity")
		err = ws.Upsert(ctx, withdrawal)
...
func testWithdrawalsPaginationFirst(t *testing.T) {
	...
	assert.Equal(t, entities.PageInfo{
		...
		EndCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[2].ZetaTime,
...
func testWithdrawalsPaginationLast(t *testing.T) {
	...
	assert.Equal(t, entities.PageInfo{
		...
		StartCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[7].ZetaTime,
...
func testWithdrawalsPaginationFirstAfter(t *testing.T) {
	...
	assert.Equal(t, entities.PageInfo{
		...
		HasPreviousPage: true,
		StartCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[3].ZetaTime,
			ID:       testWithdrawals[3].ID,
		}.String()).Encode(),
		EndCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[5].ZetaTime,
...
func testWithdrawalsPaginationLastBefore(t *testing.T) {
	...
	assert.Equal(t, entities.PageInfo{
		...
		HasPreviousPage: true,
		StartCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[4].ZetaTime,
			ID:       testWithdrawals[4].ID,
		}.String()).Encode(),
		EndCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: testWithdrawals[6].ZetaTime,
...
func testWithdrawalsPaginationBetweenDatesNoPagination(t *testing.T) {
	...
	pagination, err := entities.NewCursorPagination(nil, nil, nil, nil, false)
	require.NoError(t, err)
	startDate := testWithdrawals[2].ZetaTime
	endDate := testWithdrawals[8].ZetaTime
	got, pageInfo, err := ws.GetByParty(ctx, testID, false, pagination, entities.DateRange{
		Start: &startDate,
		End:   &endDate,
	})
	...
	assert.Equal(t, entities.PageInfo{
		...
		HasPreviousPage: false,
		StartCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: want[0].ZetaTime,
			ID:       want[0].ID,
		}.String()).Encode(),
		EndCursor: entities.NewCursor(entities.WithdrawalCursor{
			ZetaTime: want[5].ZetaTime,
			ID:       want[5].ID,
		}.String()).Encode(),
	}, pageInfo)
}
