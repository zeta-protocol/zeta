...
func getUniqueAccountTypes(accountTypes []zeta.AccountType) []zeta.AccountType {
	accountTypesList := []zeta.AccountType{}
	accountTypesMap := map[zeta.AccountType]struct{}{}
...
func transferTypeFilterToDBQuery(transferTypeFilter []entities.LedgerMovementType, args *[]interface{}) string {
	...
	if len(transferTypeFilter) > 0 {
		...
		for v := range transferTypesMap {
			_, ok := zeta.TransferType_name[int32(v)]
			if !ok {
				continue
			}
