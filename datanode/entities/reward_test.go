...
func TestRewardFromProto(t *testing.T) {
	...
	zetaTime := entities.NanosToPostgresTimestamp(now.UnixNano())

	reward, err := entities.RewardFromProto(pbReward, generateTxHash(), zetaTime, 1)
	...
	fmt.Printf("%v - %v\n", now, reward.ZetaTime)
	assert.True(t, zetaTime.Equal(reward.ZetaTime))
