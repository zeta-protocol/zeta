...
func TestProtoFromTrade(t *testing.T) {
	...
	assert.Equal(t, zetaTime.UnixNano(), p.Timestamp)
	...
	assert.Equal(t, zeta.Side_SIDE_BUY, p.Aggressor)
	assert.Equal(t, buyOrderIDString, p.BuyOrder)
	assert.Equal(t, sellOrderIDString, p.SellOrder)
	assert.Equal(t, zeta.Trade_TYPE_NETWORK_CLOSE_OUT_GOOD, p.Type)
...
func TestTradeFromProto(t *testing.T) {
	...
	testZetaTime := time.Now()
	trade, err := entities.TradeFromProto(&tradeEventProto, generateTxHash(), testZetaTime, 5)
	if err != nil {
