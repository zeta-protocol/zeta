...
func (p Proposal) ToProto() *zeta.Proposal {
	...
	var reason *zeta.ProposalError
	if p.Reason != ProposalErrorUnspecified {
		reason = ptr.From(zeta.ProposalError(p.Reason))
	}
	...
	pp := zeta.Proposal{
		...
		State:                                  zeta.Proposal_State(p.State),
...
func (pt *ProposalRationale) UnmarshalJSON(b []byte) error {
	pt.ProposalRationale = &zeta.ProposalRationale{}
...
func (pt *ProposalTerms) UnmarshalJSON(b []byte) error {
	pt.ProposalTerms = &zeta.ProposalTerms{}
	return protojson.Unmarshal(b, pt)
}
