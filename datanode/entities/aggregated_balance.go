...
// time. ZetaTime and Balance will always be set. The others will be nil unless when
...
// - zetaTime is assumed to be first
...
func AggregatedBalanceScan(fields []AccountField, rows interface {
	...
	for rows.Next() {
		...
		bal.ZetaTime, ok = values[0].(time.Time)
		if !ok {
...
func (balance *AggregatedBalance) ToProto() *v2.AggregatedBalance {
	...
	return &v2.AggregatedBalance{
		Timestamp:   balance.ZetaTime.UnixNano(),
