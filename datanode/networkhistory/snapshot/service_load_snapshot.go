...
func (b *Service) RollbackToSegment(ctx context.Context, log LoadLog, rollbackToSegment loadableSegment) error {
	...
	for _, meta := range dbMeta.TableNameToMetaData {
		if meta.Hypertable {
			result, err := tx.Exec(ctx, fmt.Sprintf("delete from %s where zeta_time > $1", meta.Name), rollbackToBlock.ZetaTime)
			if err != nil {
...
func (b *Service) loadSegment(ctx context.Context, zetaDbConn *pgx.Conn, loadLog LoadLog, segment loadableSegment, zipDir string,
	...
	startTime := time.Now()
	rowsCopied, err := copyDataIntoDatabase(ctx, zetaDbConn, reader, zipDir, dbMetaData, historyTableLastTimestamps)
	if err != nil {
...
func getLastPartitionColumnEntryForHistoryTable(ctx context.Context, zetaDbConn *pgxpool.Conn, historyTableMetaData TableMetadata) (time.Time, error) {
	...
	rows, err := zetaDbConn.Query(ctx, timeSelect)
	if err != nil {
...
func executeAllSql(ctx context.Context, zetaDbConn sqlstore.Connection, loadLog LoadLog, allSql []string) error {
	for _, sql := range allSql {
		loadLog.Infof("executing sql: %s", sql)
		_, err := zetaDbConn.Exec(ctx, sql)
		if err != nil {
...
func dropHistoryTableIndexes(ctx context.Context, zetaDbConn sqlstore.Connection, loadLog LoadLog,
	dbMetadata DatabaseMetadata,
) ([]IndexInfo, error) {
	var indexes []IndexInfo

	rows, err := zetaDbConn.Query(ctx, `select tablename, Indexname, Indexdef from pg_indexes where schemaname ='public' order by tablename`)
	if err != nil {
	...
	for _, index := range indexes {
		_, err = zetaDbConn.Exec(ctx, fmt.Sprintf("DROP INDEX %s", index.Indexname))
...
func createIndexes(ctx context.Context, zetaDbConn sqlstore.Connection,
	loadLog LoadLog, indexes []IndexInfo,
) error {
	for _, index := range indexes {
		loadLog.Infof("creating index %s", index.Indexname)
		_, err := zetaDbConn.Exec(ctx, index.Indexdef)
		if err != nil {
...
func updateContinuousAggregateDataFromHeight(ctx context.Context, conn *pgxpool.Pool, height int64) error {
	...
	for _, cagg := range dbMetaData.ContinuousAggregatesMetaData {
		...
		fromTime := fromBlock.ZetaTime.Truncate(cagg.BucketInterval)
...
func UpdateContinuousAggregateDataFromHighWaterMark(ctx context.Context, conn *pgxpool.Pool, toHeight int64) error {
	...
	for _, cagg := range dbMetaData.ContinuousAggregatesMetaData {
		...
		toTime := toBlock.ZetaTime.Truncate(cagg.BucketInterval)
