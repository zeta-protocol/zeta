...
import (
	...
	"github.com/zeta-protocol/zeta/datanode/utils/databasetest"
...
func TestMain(m *testing.M) {
	...
	zetaHome, postgresRuntimePath, err := setupDirs()
	if err != nil {
	...
	go func() {
		defer wg.Done()
		if err := runTestNode(ctx, cfg, zetaHome, stopper); err != nil {
			close(stopper)
			cfunc()
...
func runTestNode(ctx context.Context, cfg *config.Config, zetaHome string, stopper chan struct{}) error {
	...
	configWatcher, err := config.NewWatcher(context.Background(), logger, zetaPaths)
	if err != nil {
	...
	if err = cmd.Run(ctx, configWatcher, zetaPaths, []string{}); err != nil {
