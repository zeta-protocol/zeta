...
import (
	...
	v1 "github.com/zeta-protocol/zeta/wallet/network/store/v1"
...
func testNewStoreSucceeds(t *testing.T) {
	...
	s, err := v1.InitialiseStore(zetaHome)

	require.NoError(t, err)
	assert.NotNil(t, s)
	vgtest.AssertDirAccess(t, networksHome(t, zetaHome))
}
...
func testFileStoreV1SaveNetworkSucceeds(t *testing.T) {
	...
	// then
	require.NoError(t, err)
	vgtest.AssertFileAccess(t, networkPath(t, zetaHome, net.Name))

	// confirm that the network name is not saved in the file
	netpath := networkPath(t, zetaHome, net.Name)
...
func testFileStoreV1GetNetworkPathSucceeds(t *testing.T) {
	...
	// then
	assert.Equal(t, networkPath(t, zetaHome, "test"), returnedPath)
}
...
func testFileStoreV1GetNetworksPathSucceeds(t *testing.T) {
	...
	// then
	assert.Equal(t, networksHome(t, zetaHome), returnedPath)
}
...
func initialiseFromPath(t *testing.T, zetaHome paths.Paths) *v1.FileStore {
	t.Helper()
	s, err := v1.InitialiseStore(zetaHome)
	if err != nil {
...
func networksHome(t *testing.T, zetaHome *paths.CustomPaths) string {
	t.Helper()
	return zetaHome.ConfigPathFor(paths.WalletServiceNetworksConfigHome)
}

func networkPath(t *testing.T, zetaHome *paths.CustomPaths, name string) string {
	t.Helper()
	return filepath.Join(networksHome(t, zetaHome), name+".toml")
}
