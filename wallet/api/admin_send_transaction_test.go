...
import (
	...
	"github.com/zeta-protocol/zeta/wallet/api/mocks"
	walletnode "github.com/zeta-protocol/zeta/wallet/api/node"
	nodemocks "github.com/zeta-protocol/zeta/wallet/api/node/mocks"
	"github.com/zeta-protocol/zeta/wallet/api/node/types"
...
func testAdminSendingTransactionWithMalformedTransactionFails(t *testing.T) {
	...
	// then
	assertInvalidParams(t, errorDetails, errors.New("the transaction does not use a valid Zeta command: unknown field \"bob\" in zeta.wallet.v1.SubmitTransactionRequest"))
	assert.Empty(t, result)
}
...
func testAdminSendingTransactionWithInvalidTransactionFails(t *testing.T) {
	...
	// then
	assertInvalidParams(t, errorDetails, fmt.Errorf("vote_submission.proposal_id (should be a valid zeta ID)"))
	assert.Empty(t, result)
}
