...
func testSendingTransactionWithInvalidParamsFails(t *testing.T) {
	tcs := []struct {
		...
		{
			name: "with transaction as invalid Zeta command",
			params: api.ClientSendTransactionParams{
				PublicKey:   vgrand.RandomStr(10),
				SendingMode: "TYPE_SYNC",
				Transaction: map[string]interface{}{
					"type": "not zeta command",
				},
			},
			expectedError: errors.New("the transaction does not use a valid Zeta command: unknown field \"type\" in zeta.wallet.v1.SubmitTransactionRequest"),
		},
	}
