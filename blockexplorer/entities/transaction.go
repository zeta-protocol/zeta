...
import (
	...
	pb "github.com/zeta-protocol/zeta/protos/blockexplorer/api/v1"
)
...
func (t *TxResultRow) ToProto() (*pb.Transaction, error) {
	...
	if err := proto.Unmarshal(txResult.Tx, &cTx); err != nil {
		return nil, fmt.Errorf("unmarshalling zeta transaction: %w", err)
	}

	inputData, err := commands.UnmarshalInputData(cTx.InputData)
	if err != nil {
		return nil, fmt.Errorf("unmarshalling zeta input data: %w", err)
	}
