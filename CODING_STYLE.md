...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/foo_buffer_mock.go -package mocks github.com/zeta-protocol/zeta/foo FooBuffer
...
```go
//go:generate go run github.com/golang/mock/mockgen -destination mocks/some_dependency_mock.go -package mocks github.com/zeta-protocol/zeta/pkg SomeDependency
...
* Message types representing a unit of data, currently used in the core (e.g. `Order`, `Market`, `Transfer`, ...) are defined in the `proto/zeta.proto` file. These types are imported under the alias `types`.
...
message GetSomethingsByMarketIDResponse {
    repeated zeta.Something something = 1;
}
```
