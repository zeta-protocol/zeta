...
import (
	...
	zetacontext "github.com/zeta-protocol/zeta/libs/context"
	...
	"github.com/zeta-protocol/zeta/core/collateral"
	"github.com/zeta-protocol/zeta/core/events"
	"github.com/zeta-protocol/zeta/core/execution"
	"github.com/zeta-protocol/zeta/core/execution/mocks"
	...
	"github.com/zeta-protocol/zeta/core/monitor"
	"github.com/zeta-protocol/zeta/core/oracles"
	...
	proto "github.com/zeta-protocol/zeta/protos/zeta"
...
func (tm *testMarket) WithSubmittedLiquidityProvision(t *testing.T, party string, amount uint64, fee string, buys, sells []*types.LiquidityOrder) *testMarket {
	t.Helper()
	ctx := zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash())
...
func TestMarketClosing(t *testing.T) {
	...
	tm.now = closingAt
	closed := tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingAt)
...
func TestMarketClosingAfterUpdate(t *testing.T) {
	...
	closed := tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus1Sec)
	...
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus2Sec)
	...
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), closingTimePlus3Sec)
...
func TestLiquidityFeeWhenTargetStakeDropsDueToFlowOfTime(t *testing.T) {
	...
	tm.now = now
	tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), now)
...
func TestTriggerByPriceNoTradesInAuction(t *testing.T) {
	...
	tm.now = openEnd
	tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), openEnd)
	now = openEnd
	...
	closed := tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), auctionEndTime)
	assert.False(t, closed)

	tm.now = afterAuction
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterAuction)
...
func TestTriggerByPriceAuctionPriceOutsideBounds(t *testing.T) {
	...
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), initialAuctionEnd)
	assert.False(t, closed)
...
func TestTriggerByMarketOrder(t *testing.T) {
	...
	// now leave auction
	tm.now = openingEnd
	tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), openingEnd)
	now = openingEnd
	...
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), tm.now)
...
func TestTriggerAfterOpeningAuction(t *testing.T) {
	...
	closed := tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterOpeningAuction)
	assert.False(t, closed)
	auctionEnd = tm.market.GetMarketData().AuctionEnd
	require.Equal(t, int64(0), auctionEnd) // Not in auction
	...
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), pMonitorAuctionEndTime)
	assert.False(t, closed)

	tm.now = afterPMonitorAuction
	closed = tm.market.OnTick(zetacontext.WithTraceID(context.Background(), vgcrypto.RandomHash()), afterPMonitorAuction)
	assert.False(t, closed)
}
...
func TestLPOrdersRollback(t *testing.T) {
	...
	t.Run("LiquidityProvision_REJECTED", func(t *testing.T) {
		...
		var found *zetapb.LiquidityProvision
		for _, e := range tm.events {
			switch evt := e.(type) {
...
func Test3008And3007CancelLiquidityProvision(t *testing.T) {
	...
	t.Run("Fee are distribute to party-2 only", func(t *testing.T) {
		var found []*zetapb.LedgerMovement
		for _, e := range tm.events {
			switch evt := e.(type) {
