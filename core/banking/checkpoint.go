...
func (e *Engine) loadAssetActions(aa []*types.AssetAction) {
	for _, v := range aa {
		...
		if v.BuiltinD != nil {
			e.deposits[v.ID] = e.newDeposit(v.ID, v.BuiltinD.PartyID, v.BuiltinD.ZetaAssetID, v.BuiltinD.Amount, v.Hash)
		} else if v.Erc20D != nil {
			e.deposits[v.ID] = e.newDeposit(v.ID, v.Erc20D.TargetPartyID, v.Erc20D.ZetaAssetID, v.Erc20D.Amount, v.Hash)
		}
	}
}
