...
func isSimilar(dispatchStrategy1, dispatchStrategy2 *zetapb.DispatchStrategy) bool {
		...
		if v.DispatchStrategy == nil {
			...
			if _, err = e.ensureFeeForTransferFunds(amount, v.From, v.Asset, v.FromAccountType); err == nil {
				...
				for _, fms := range marketScores {
					...
					if v.DispatchStrategy.Metric == zetapb.DispatchMetric_DISPATCH_METRIC_MARKET_VALUE && fms.Score.IsPositive() {
