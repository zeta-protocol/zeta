...
func (e *Engine) EnableERC20(
	...
	asset, _ := e.assets.Get(al.ZetaAssetID)
	if _, ok := e.assetActs[al.ZetaAssetID]; ok {
		e.log.Error("asset already being listed", logging.AssetID(al.ZetaAssetID))
...
func (e *Engine) UpdateERC20(
	...
	asset, err := e.assets.Get(event.ZetaAssetID)
	if err != nil {
		e.log.Panic("couldn't retrieve the ERC20 asset",
			logging.AssetID(event.ZetaAssetID),
		)
	}
	aa := &assetAction{
		id:                      id,
