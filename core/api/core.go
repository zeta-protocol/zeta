...
import (
	...
	"github.com/zeta-protocol/zeta/wallet/crypto"
...
func (s *coreService) GetZetaTime(ctx context.Context, _ *protoapi.GetZetaTimeRequest) (*protoapi.GetZetaTimeResponse, error) {
	...
	return &protoapi.GetZetaTimeResponse{
...
func (s *coreService) Statistics(ctx context.Context, _ *protoapi.StatisticsRequest) (*protoapi.StatisticsResponse, error) {
	...
	genesisTime := ""
	if gt != nil {
		genesisTime = zetatime.Format(*gt)
	}
	...
	stats := &protoapi.Statistics{
		...
		CurrentTime:           zetatime.Format(time.Now()),
		ZetaTime:              zetatime.Format(epochTime),
		Uptime:                zetatime.Format(s.stats.GetUptime()),
		...
		EpochStartTime:        zetatime.Format(s.stats.GetEpochStartTime()),
		EpochExpiryTime:       zetatime.Format(s.stats.GetEpochExpireTime()),
		ChainId:               chainID,
	}
