...
func protocolUpgradeProposalID(upgradeBlockHeight uint64, zetaReleaseTag string) string {
	return fmt.Sprintf("%v@%v", zetaReleaseTag, upgradeBlockHeight)
}
...
func (e *Engine) IsValidProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, zetaReleaseTag string) error {
	...
	newv, err := semver.Parse(TrimReleaseTag(zetaReleaseTag))
	if err != nil {
		err = fmt.Errorf("invalid protocol version for upgrade received: version (%s), %w", zetaReleaseTag, err)
...
func (e *Engine) UpgradeProposal(ctx context.Context, pk string, upgradeBlockHeight uint64, zetaReleaseTag string) error {
	...
	if err := e.IsValidProposal(ctx, pk, upgradeBlockHeight, zetaReleaseTag); err != nil {
		return err
	}

	ID := protocolUpgradeProposalID(upgradeBlockHeight, zetaReleaseTag)
	...
	if zetaReleaseTag != e.currentVersion {
...
func (e *Engine) sendAndKeepEvent(ctx context.Context, ID string, activeProposal *protocolUpgradeProposal) {
	...
	evt := events.NewProtocolUpgradeProposalEvent(ctx, activeProposal.blockHeight, activeProposal.zetaReleaseTag, activeProposal.approvers(), status)
	evtProto := evt.Proto()
...
func (e *Engine) BeginBlock(ctx context.Context, blockHeight uint64) {
	...
	if accepted != nil {
		e.upgradeStatus.AcceptedReleaseInfo = &types.ReleaseInfo{
			ZetaReleaseTag:     accepted.zetaReleaseTag,
...
func (e *Engine) Cleanup(ctx context.Context) {
	...
	for _, ID := range e.getProposalIDs() {
		...
		e.broker.Send(events.NewProtocolUpgradeProposalEvent(ctx, pup.blockHeight, pup.zetaReleaseTag, pup.approvers(), status))
		delete(e.activeProposals, ID)
		delete(e.events, ID)
	}
}
...
func (e *Engine) SetCoreReadyForUpgrade() {
	...
	e.log.Info("marking zeta core as ready to shut down")
...
func (e *Engine) SetReadyForUpgrade() {
	...
	e.log.Info("marking zeta core and data node as ready to shut down")
