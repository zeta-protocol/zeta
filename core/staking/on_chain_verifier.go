...
func (o *OnChainVerifier) CheckStakeDeposited(
	...
	decodedPubKeySlice, err := hex.DecodeString(event.ZetaPubKey)
	if err != nil {
	...
	for _, address := range o.stakingBridgeAddresses {
		...
		zetaPubKey := strings.TrimPrefix(event.ZetaPubKey, "0x")
		...
		for iter.Next() {
			...
			if hex.EncodeToString(iter.Event.ZetaPublicKey[:]) == zetaPubKey &&
