...
import (
	...
	"github.com/zeta-protocol/zeta/core/collateral/mocks"
...
func TestCheckPointLoadingWithAlias(t *testing.T) {
	...
	ab := []*checkpoint.AssetBalance{
		{Party: "*", Asset: "ZETA", Balance: "1000"},
		{Party: "*ACCOUNT_TYPE_GLOBAL_REWARD", Asset: "ZETA", Balance: "2000"},
	}
	...
	acc, err := e.GetGlobalRewardAccount("ZETA")
	require.NoError(t, err)
	require.Equal(t, "3000", acc.Balance.String())

	_, err = e.GetPartyGeneralAccount("*ACCOUNT_TYPE_GLOBAL_REWARD", "ZETA")
	require.Error(t, err)
}
