...
import (
	...
	bridge "github.com/zeta-protocol/zeta/core/contracts/erc20_bridge_logic_restricted"
	multisig "github.com/zeta-protocol/zeta/core/contracts/multisig_control"
...
// Assets ...
//
//go:generate go run github.com/golang/mock/mockgen -destination mocks/assets_mock.go -package mocks github.com/zeta-protocol/zeta/core/evtforward/ethereum Assets
...
// LogFilterer wraps the Ethereum event filterers to return Zeta events.
...
func (f *LogFilterer) debugAssetDeposited(event *bridge.Erc20BridgeLogicRestrictedAssetDeposited) {
	if f.log.IsDebug() {
		f.log.Debug("Found AssetDeposited event",
			logging.String("bridge-address", f.collateralBridge.HexAddress()),
			logging.String("user-ethereum-address", event.UserAddress.Hex()),
			logging.String("user-zeta-address", hex.EncodeToString(event.ZetaPublicKey[:])),
			logging.String("asset-id", event.AssetSource.Hex()),
		)
	}
}
			...
			Erc20: &vgproto.ERC20Event{
				...
				Action: &vgproto.ERC20Event_AssetDelist{
					AssetDelist: &vgproto.ERC20AssetDelist{
						ZetaAssetId: event.AssetSource.Hex(),
					},
				},
			},
		},
