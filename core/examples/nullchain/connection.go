...
import (
	...
	"github.com/zeta-protocol/zeta/core/examples/nullchain/config"
...
func (c *Connection) ZetaTime() (time.Time, error) {
	...
	gvtReq := &v2.GetZetaTimeRequest{}
	response, err := c.datanode.GetZetaTime(ctx, gvtReq)
	if err != nil {
...
func (c *Connection) GetProposalsByParty(party *Party) ([]*zeta.GovernanceData, error) {
	...
	var proposals []*zeta.GovernanceData
