...
import (
	...
	zetactx "github.com/zeta-protocol/zeta/libs/context"
	...
	snappb "github.com/zeta-protocol/zeta/protos/zeta/snapshot/v1"
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_mock.go -package mocks github.com/zeta-protocol/zeta/core/snapshot TimeService
type TimeService interface {
	GetTimeNow() time.Time
	SetTimeNow(context.Context, time.Time)
}

//go:generate go run github.com/golang/mock/mockgen -destination mocks/stats_mock.go -package mocks github.com/zeta-protocol/zeta/core/snapshot StatsService
...
func New(ctx context.Context, zetapath paths.Paths, conf Config, log *logging.Logger, tm TimeService, stats StatsService) (*Engine, error) {
	...
	dbPath, err := conf.validate(zetapath)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) snapshotNow(ctx context.Context, saveAsync bool) ([]byte, error) {
	...
	appUpdate := false
	height, err := zetactx.BlockHeightFromContext(ctx)
	if err != nil {
		e.avlLock.Unlock()
		return nil, err
	}
	...
	_, block := zetactx.TraceIDFromContext(ctx)
	...
	cid, err := zetactx.ChainIDFromContext(ctx)
	if err != nil {
		e.avlLock.Unlock()
		return nil, err
	}
