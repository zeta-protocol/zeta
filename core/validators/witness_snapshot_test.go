...
func TestSnapshot(t *testing.T) {
	for i := 0; i < 100; i++ {
		...
		erc2.top.EXPECT().SelfZetaPubKey().AnyTimes().Return("1234")
		...
		erc2.top.EXPECT().IsValidatorZetaPubKey(pubkey.Hex()).Times(1).Return(true)
		...
		erc3.top.EXPECT().SelfZetaPubKey().AnyTimes().Return("1234")
