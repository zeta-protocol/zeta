...
func TestTopologySnapshot(t *testing.T) {
	...
	nr2 := commandspb.AnnounceNode{
		Id:              "zeta-master-pubkey-2",
		ChainPubKey:     tmPubKeys[1],
		ZetaPubKey:      hexEncode("zeta-key-2"),
		EthereumAddress: "0x6d53C489bbda35B8096C8b4Cb362e2889F82E19B",
	}
	...
	kr1 := &commandspb.KeyRotateSubmission{
		...
		CurrentPubKeyHash: hashKey("zeta-key"),
	}
	...
	kr2 := &commandspb.KeyRotateSubmission{
		NewPubKeyIndex:    1,
		TargetBlock:       11,
		NewPubKey:         "new-zeta-key-2",
		CurrentPubKeyHash: hashKey("zeta-key-2"),
	}
	...
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("zeta-key"), ekr1, MockVerify)
	assert.NoError(t, err)
	...
	err = top.ProcessEthereumKeyRotation(ctx, hexEncode("zeta-key-2"), ekr2, MockVerify)
	assert.NoError(t, err)
	...
	ctx = zetactx.WithBlockHeight(ctx, 100)
