...
func TestProcessValidatorHeartbeat(t *testing.T) {
	...
	cmd.ZetaSignature = &commandspb.Signature{
		Value: "haha",
	}
	...
	cmd.ZetaSignature.Value = "abcdef"

	topology.validators["node1"].data.ZetaPubKey = "fooo"
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, nil, nil))
	...
	topology.validators["node1"].data.ZetaPubKey = "eeee"
	rejectZeta := func(message, signature, pubkey []byte) error { return errors.New("unverifiable zeta signature") }
	rejectEth := func(message, signature []byte, hexAddress string) error {
		return errors.New("unverifiable eth signature")
	}
	acceptZeta := func(message, signature, pubkey []byte) error { return nil }
	acceptEth := func(message, signature []byte, hexAddress string) error { return nil }

	// unverifiable zeta signature
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, rejectZeta, nil))
	...
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptZeta, nil))
	...
	require.Error(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptZeta, rejectEth))
	...
	require.NoError(t, topology.ProcessValidatorHeartbeat(context.Background(), cmd, acceptZeta, acceptEth))
