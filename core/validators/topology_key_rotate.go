...
func (t *Topology) AddKeyRotate(ctx context.Context, nodeID string, currentBlockHeight uint64, kr *commandspb.KeyRotateSubmission) error {
	...
	if node.data.ZetaPubKeyIndex >= kr.NewPubKeyIndex {
	...
	if hashedZetaPubKey != kr.CurrentPubKeyHash {
...
func (t *Topology) keyRotationBeginBlockLocked(ctx context.Context) {
	...
	for _, nodeID := range nodeIDs {
		...
		if !ok {
			...
			t.log.Error("failed to rotate Zeta key due to non present validator", logging.String("nodeID", nodeID))
			continue
		}

		oldPubKey := data.data.ZetaPubKey
		rotation := t.pendingPubKeyRotations[t.currentBlockHeight][nodeID]

		data.data.ZetaPubKey = rotation.newPubKey
		data.data.ZetaPubKeyIndex = rotation.newKeyIndex
		t.validators[nodeID] = data

		t.notifyKeyChange(ctx, oldPubKey, rotation.newPubKey)
		t.broker.Send(events.NewZetaKeyRotationEvent(ctx, nodeID, oldPubKey, rotation.newPubKey, t.currentBlockHeight))
