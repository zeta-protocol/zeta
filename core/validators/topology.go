...
import (
	...
	v1 "github.com/zeta-protocol/zeta/protos/zeta/snapshot/v1"
...
func (v ValidatorData) IsValid() bool {
	if len(v.ID) <= 0 || len(v.ZetaPubKey) <= 0 ||
...
func (t *Topology) AllZetaPubKeys() []string {
	...
	for _, data := range t.validators {
		keys = append(keys, data.data.ZetaPubKey)
	}
	return keys
}
...
func (t *Topology) SelfZetaPubKey() string {
	...
	return t.wallets.GetZeta().PubKey().Hex()
}
...
func (t *Topology) SelfNodeID() string {
	...
	return t.wallets.GetZeta().ID().Hex()
}
...
func (t *Topology) IsValidatorZetaPubKey(pubkey string) (ok bool) {
	defer func() {
		if t.log.GetLevel() <= logging.DebugLevel {
			...
			t.log.Debug(s,
				logging.Strings("validators", t.AllZetaPubKeys()),
	...
	for _, data := range t.validators {
		if data.data.ZetaPubKey == pubkey {
			return true
		}
	}
...
func (t *Topology) IsSelfTendermintValidator() bool {
	return t.IsTendermintValidator(t.SelfZetaPubKey())
}
...
func (t *Topology) GetVotingPower(pubkey string) int64 {
	...
	defer t.mu.RUnlock()
	for _, data := range t.validators {
		if data.data.ZetaPubKey == pubkey && data.status == ValidatorStatusTendermint {
...
func (t *Topology) AddNewNode(ctx context.Context, nr *commandspb.AnnounceNode, status ValidatorStatus) error {
	...
	if _, ok := t.validators[nr.Id]; ok {
		return ErrZetaNodeAlreadyRegisterForChain
	}
	...
	t.log.Info("new node registration successful",
		...
		logging.String("zeta-key", nr.ZetaPubKey),
...
func (t *Topology) LoadValidatorsOnGenesis(ctx context.Context, rawstate []byte) (err error) {
	...
	// tm is base64 encoded, zeta is hex
...
func (t *Topology) checkValidatorDataWithSelfWallets(data ValidatorData) {
	...
	if hasError {
		t.log.Panic("invalid node wallet configurations, the genesis validator mapping differ to the wallets imported by the nodewallet",
			...
			logging.String("genesis-zeta-pubkey", data.ZetaPubKey),
			logging.String("nodewallet-zeta-pubkey", t.wallets.GetZeta().PubKey().Hex()),
			logging.String("genesis-zeta-id", data.ID),
			logging.String("nodewallet-zeta-id", t.wallets.GetZeta().ID().Hex()),
