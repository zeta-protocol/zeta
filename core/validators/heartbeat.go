...
import (
	...
	"github.com/zeta-protocol/zeta/core/nodewallets/eth/clef"
...
func (t *Topology) ProcessValidatorHeartbeat(ctx context.Context, vh *commandspb.ValidatorHeartbeat,
	...
	zetas, err := hex.DecodeString(vh.GetZetaSignature().Value)
	if err != nil {
		validator.heartbeatTracker.recordHeartbeatResult(false)
		return err
	}
	zetaPubKey, err := hex.DecodeString(node.data.ZetaPubKey)
	if err != nil {
		validator.heartbeatTracker.recordHeartbeatResult(false)
		return err
	}
...
func (t *Topology) prepareHeartbeat(blockHash string) *commandspb.ValidatorHeartbeat {
	...
	zetaSignature, err := t.wallets.GetZeta().Sign(blockHashBytes)
	if err != nil {
		t.log.Error("could not sign heartbeat with zeta wallet",
	...
	return &commandspb.ValidatorHeartbeat{
		NodeId: t.SelfNodeID(),
		ZetaSignature: &commandspb.Signature{
			Value: hex.EncodeToString(zetaSignature),
			Algo:  t.wallets.GetZeta().Algo(),
		},
