...
import (
	...
	"github.com/zeta-protocol/zeta/core/governance/mocks"
...
func produceTimeTriggeredDataSourceSpec(termTimestamp time.Time) (*types.DataSourceSpecFilter, *types.DataSourceSpecBindingForFuture) {
	return &types.DataSourceSpecFilter{
		...
		&types.DataSourceSpecBindingForFuture{
			SettlementDataProperty:     "prices.ETH.value",
			TradingTerminationProperty: "zetaprotocol.builtin.timestamp",
		}
}
...
func newMarketTerms(termFilter *types.DataSourceSpecFilter, termBinding *types.DataSourceSpecBindingForFuture, termExt bool) *types.ProposalTermsNewMarket {
	...
	if termExt {
		...
		dt = types.NewDataSourceDefinition(zetapb.DataSourceDefinitionTypeExt).SetOracleConfig(
		...
		dt = types.NewDataSourceDefinition(zetapb.DataSourceDefinitionTypeInt).SetTimeTriggerConditionConfig(
