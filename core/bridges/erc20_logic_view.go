...
import (
	...
	erc20contract "github.com/zeta-protocol/zeta/core/contracts/erc20"
	bridgecontract "github.com/zeta-protocol/zeta/core/contracts/erc20_bridge_logic_restricted"
	"github.com/zeta-protocol/zeta/core/metrics"
	"github.com/zeta-protocol/zeta/core/types"
	vgerrors "github.com/zeta-protocol/zeta/libs/errors"
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/eth_client_mock.go -package mocks github.com/zeta-protocol/zeta/core/bridges ETHClient
...
//go:generate go run github.com/golang/mock/mockgen -destination mocks/eth_confirmations_mock.go -package mocks github.com/zeta-protocol/zeta/core/bridges EthConfirmations
...
func (e *ERC20LogicView) FindAssetList(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_asset_list", al.ZetaAssetID, resp)
	}()
	...
	assetID := strings.TrimPrefix(al.ZetaAssetID, "0x")

	for iter.Next() {
		if hex.EncodeToString(iter.Event.ZetaAssetId[:]) == assetID &&
...
func (e *ERC20LogicView) FindDeposit(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_deposit", d.ZetaAssetID, resp)
	}()
	...
	for iter.Next() {
		if hex.EncodeToString(iter.Event.ZetaPublicKey[:]) == targetPartyID &&
...
func (e *ERC20LogicView) FindWithdrawal(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_withdrawal", w.ZetaAssetID, resp)
	}()
...
func (e *ERC20LogicView) FindAssetLimitsUpdated(
	...
	resp := "ok"
	defer func() {
		metrics.EthCallInc("find_asset_limits_updated", update.ZetaAssetID, resp)
	}()
