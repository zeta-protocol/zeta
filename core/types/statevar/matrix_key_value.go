...
func (dm DecimalMatrix) ToProto() *zeta.StateVarValue {
	rows := make([]*zeta.VectorValue, 0, len(dm.Val))
	for _, fvi := range dm.Val {
		fviAsString := make([]string, 0, len(fvi))
		for _, dv := range fvi {
			fviAsString = append(fviAsString, dv.String())
		}
		rows = append(rows, &zeta.VectorValue{
			Value: fviAsString,
		})
	}
	return &zeta.StateVarValue{
		Value: &zeta.StateVarValue_MatrixVal{
			MatrixVal: &zeta.MatrixValue{
				Value: rows,
			},
		},
	}
