...
func (kvb KeyValueBundle) ToProto() []*zeta.KeyValueBundle {
	res := make([]*zeta.KeyValueBundle, 0, len(kvb.KVT))
	for _, kvt := range kvb.KVT {
		res = append(res, &zeta.KeyValueBundle{
			Key:       kvt.Key,
...
func KeyValueBundleFromProto(protoKVT []*zeta.KeyValueBundle) (*KeyValueBundle, error) {
...
func ValueFromProto(val *zeta.StateVarValue) (value, error) {
	if val == nil {
	...
	case *zeta.StateVarValue_MatrixVal:
