...
import (
	...
	storev1 "github.com/zeta-protocol/zeta/wallet/wallet/store/v1"
...
func InitialiseWalletLoader(zetaPaths paths.Paths) (*WalletLoader, error) {
	walletHome, err := zetaPaths.CreateDataDirFor(paths.ZetaNodeWalletsDataHome)
	if err != nil {
		return nil, fmt.Errorf("couldn't get the directory path for %s: %w", paths.ZetaNodeWalletsDataHome, err)
	}
...
func (l *WalletLoader) Generate(passphrase string) (*Wallet, map[string]string, error) {
	...
	walletName := fmt.Sprintf("zeta.%v", time.Now().UnixNano())
...
func (l *WalletLoader) Import(sourceFilePath string, passphrase string) (*Wallet, map[string]string, error) {
	...
	w, err := importSourceWallet(ctx, sourcePath, sourceWalletName, passphrase)
	if err != nil {
		return nil, nil, err
	}
	...
	destWalletName := fmt.Sprintf("zeta.%v", time.Now().UnixNano())
...
func importSourceWallet(ctx context.Context, sourcePath string, sourceWalletName string, passphrase string) (wallet.Wallet, error) {
	...
	if err := sourceStore.UnlockWallet(ctx, sourceWalletName, passphrase); err != nil {
		return nil, fmt.Errorf("couldn't unlock the source wallet: %w", err)
	}

	w, err := sourceStore.GetWallet(ctx, sourceWalletName)
	if err != nil {
		return nil, fmt.Errorf("couldn't get source wallet %s: %w", sourceWalletName, err)
	}
	return w, nil
}
...
func newWallet(loader loader, store *storev1.FileStore, walletName, passphrase string) (*Wallet, error) {
	...
	if keyPairCount := len(keyPairs); keyPairCount == 0 {
		return nil, fmt.Errorf("zeta wallet for node requires to have 1 key pair, none found")
	} else if keyPairCount != 1 {
		return nil, fmt.Errorf("zeta wallet for node requires to have max 1 key pair, found %v", keyPairCount)
	}
