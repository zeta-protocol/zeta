...
type RegisteredZetaWallet struct {
	Name       string `json:"name"`
	Passphrase string `json:"passphrase"`
}
...
func NewLoader(zetaPaths paths.Paths, passphrase string) (*Loader, error) {
	registryFilePath, err := zetaPaths.CreateConfigPathFor(paths.NodeWalletsConfigFile)
	if err != nil {
