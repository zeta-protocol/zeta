...
func changeOracleSpec(spec *types.DataSourceSpec) {
	...
	spec.Status = zetapb.DataSourceSpec_STATUS_UNSPECIFIED
}
...
func TestMarketDeepClone(t *testing.T) {
	...
	pme := zetapb.Market{
		Id: "Id",
		TradableInstrument: &zetapb.TradableInstrument{
			Instrument: &zetapb.Instrument{
				Id:   "Id",
				Code: "Code",
				Name: "Name",
				Metadata: &zetapb.InstrumentMetadata{
					Tags: []string{"Tag1", "Tag2"},
				},
				Product: &zetapb.Instrument_Future{
					Future: &zetapb.Future{
						SettlementAsset: "Asset",
						QuoteName:       "QuoteName",
						DataSourceSpecForSettlementData: &zetapb.DataSourceSpec{
							Id:        "Id",
							CreatedAt: 1000,
							UpdatedAt: 2000,
						...
						DataSourceSpecForTradingTermination: &zetapb.DataSourceSpec{
							Id:        "Id2",
							CreatedAt: 1000,
							UpdatedAt: 2000,
							Data: &zetapb.DataSourceDefinition{
						...
						DataSourceSpecBinding: &zetapb.DataSourceSpecToFutureBinding{
			...
			MarginCalculator: &zetapb.MarginCalculator{
				ScalingFactors: &zetapb.ScalingFactors{
					SearchLevel:       123.45,
					InitialMargin:     234.56,
					CollateralRelease: 345.67,
				},
			},
			RiskModel: &zetapb.TradableInstrument_SimpleRiskModel{
				SimpleRiskModel: &zetapb.SimpleRiskModel{
					Params: &zetapb.SimpleModelParams{
						FactorLong:           123.45,
						FactorShort:          234.56,
						MaxMoveUp:            345.67,
						MinMoveDown:          456.78,
		...
		DecimalPlaces: 5,
		Fees: &zetapb.Fees{
			Factors: &zetapb.FeeFactors{
				MakerFee:          "0.1",
				InfrastructureFee: "0.2",
				LiquidityFee:      "0.3",
			},
		},
		OpeningAuction: &zetapb.AuctionDuration{
			Duration: 1000,
			Volume:   2000,
		},
		PriceMonitoringSettings: &zetapb.PriceMonitoringSettings{
			Parameters: &zetapb.PriceMonitoringParameters{
				Triggers: []*zetapb.PriceMonitoringTrigger{
					{
						Horizon:          1000,
						Probability:      "123.45",
		...
		LiquidityMonitoringParameters: &zetapb.LiquidityMonitoringParameters{
			TargetStakeParameters: &zetapb.TargetStakeParameters{
				TimeWindow:    1000,
				ScalingFactor: 2.0,
			},
		...
		TradingMode: zetapb.Market_TRADING_MODE_CONTINUOUS,
		State:       zetapb.Market_STATE_ACTIVE,
		MarketTimestamps: &zetapb.MarketTimestamps{
			Proposed: 1000,
			Pending:  2000,
			Open:     3000,
			Close:    4000,
	...
	me.TradingMode = zetapb.Market_TRADING_MODE_UNSPECIFIED
	me.State = zetapb.Market_STATE_UNSPECIFIED
