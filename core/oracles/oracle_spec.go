...
// Currently ZETA network utilises internal triggers in the oracle function path, even though
...
func NewOracleSpec(originalSpec types.ExternalDataSourceSpec) (*OracleSpec, error) {
	...
	for _, f := range filtersFromSpec {
		if isExtType {
			...
			if strings.HasPrefix(f.Key.Name, "zetaprotocol.builtin") && f.Key.Type == datapb.PropertyKey_TYPE_TIMESTAMP {
				builtInKey = true
			}
			...
			// Currently ZETA network uses only one type of internal data source - time triggered
			// that uses the property name "zetaprotocol.builtin.timestamp"
			// https://github.com/zetaprotocol/specs/blob/master/protocol/0048-DSRI-data_source_internal.md#13-zeta-time-changed
			...
			typedFilters[f.Key.Name] = &filter{
				propertyName: "zetaprotocol.builtin.timestamp",
