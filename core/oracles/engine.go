...
//
//go:generate go run github.com/golang/mock/mockgen -destination mocks/time_service_mock.go -package mocks github.com/zeta-protocol/zeta/core/oracles TimeService
...
func (e *Engine) sendNewOracleSpecSubscription(ctx context.Context, update updatedSubscription) {
	proto := &zetapb.ExternalDataSourceSpec{
	...
	e.broker.Send(events.NewOracleSpecEvent(ctx, zetapb.OracleSpec{ExternalDataSourceSpec: proto}))
}
...
func (e *Engine) sendMatchedOracleData(ctx context.Context, data OracleData, specIDs []OracleSpecID) {
	...
	e.broker.Send(events.NewOracleDataEvent(ctx, zetapb.OracleData{ExternalData: dataProto.ExternalData}))
}
