...
import (
	...
	bmok "github.com/zeta-protocol/zeta/core/broker/mocks"
	"github.com/zeta-protocol/zeta/core/events"
	"github.com/zeta-protocol/zeta/core/oracles"
	"github.com/zeta-protocol/zeta/core/oracles/mocks"
...
func spec(t *testing.T, currency string, op datapb.Condition_Operator, price string, keys ...string) specBundle {
	...
	testSpec := zetapb.NewDataSourceSpec(
		zetapb.NewDataSourceDefinition(
...
func (b *testBroker) expectNewOracleSpecSubscription(currentTime time.Time, spec *types.OracleSpec) {
	...
	proto.Spec.Status = zetapb.DataSourceSpec_STATUS_ACTIVE
	b.EXPECT().Send(events.NewOracleSpecEvent(b.ctx, zetapb.OracleSpec{ExternalDataSourceSpec: proto})).Times(1)
}

func (b *testBroker) expectOracleSpecSubscriptionDeactivation(currentTime time.Time, spec *types.OracleSpec) {
	proto := spec.ExternalDataSourceSpec.IntoProto()
	proto.Spec.CreatedAt = currentTime.UnixNano()
	proto.Spec.Status = zetapb.DataSourceSpec_STATUS_DEACTIVATED
	b.EXPECT().Send(events.NewOracleSpecEvent(b.ctx, zetapb.OracleSpec{ExternalDataSourceSpec: proto})).Times(1)
}

func (b *testBroker) expectMatchedOracleDataEvent(currentTime time.Time, data *zetapb.OracleData, specIDs []string) {
