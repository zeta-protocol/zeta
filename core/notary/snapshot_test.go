...
func TestNotarySnapshotRoundTrip(t *testing.T) {
	...
	notr.top.EXPECT().IsValidator().AnyTimes().Return(true)
	notr.top.EXPECT().SelfZetaPubKey().AnyTimes().Return("123456")
	...
	snapNotr.top.EXPECT().SelfZetaPubKey().AnyTimes().Return("123456")
	snapNotr.top.EXPECT().IsValidatorZetaPubKey(gomock.Any()).AnyTimes().Return(true)
