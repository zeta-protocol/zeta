...
import (
	...
	"github.com/zeta-protocol/zeta/visor/config"
	"github.com/zeta-protocol/zeta/visor/github"
	"github.com/zeta-protocol/zeta/visor/utils"
)
...
func (v *Visor) installUpgradeFolder(ctx context.Context, folder, releaseTag string, conf config.AutoInstallConfig) error {
	...
	if conf.Asset.Name == "" {
		return missingAutoInstallAssetError("zeta")
	}
	...
	runConf.Zeta.Binary.Path = conf.Asset.GetBinaryPath()

	if runConf.DataNode != nil {
		runConf.DataNode.Binary.Path = conf.Asset.GetBinaryPath()

		if len(runConf.DataNode.Binary.Args) != 0 && runConf.DataNode.Binary.Args[0] != zetaDataNodeStartCmdArgs[0] {
			runConf.DataNode.Binary.Args = append(zetaDataNodeStartCmdArgs, runConf.DataNode.Binary.Args[1:]...)
		}
	}
...
func (v *Visor) prepareNextUpgradeFolder(ctx context.Context, releaseTag string) error {
	v.log.Debug("preparing next upgrade folder",
		logging.String("zetaTagVersion", releaseTag),
	)
