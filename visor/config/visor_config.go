				...
				[autoInstall]
					...
					[autoInstall.assets]
						[autoInstall.assets.zeta]
							asset_name = "zeta-darwin-amd64.zip"
							binary_name = "zeta"
	...
	return &VisorConfig{
		...
		data: &VisorConfigFile{
			...
			AutoInstall: AutoInstallConfig{
				Enabled:               true,
				GithubRepositoryOwner: "zetaprotocol",
				GithubRepository:      "zeta",
				Asset: AssetsConfig{
					Name:       fmt.Sprintf("zeta-%s-%s.zip", runtime.GOOS, "amd64"),
					BinaryName: toPointer("zeta"),
				},
			},
		},
	}
