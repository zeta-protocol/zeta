...
import (
	...
	"github.com/zeta-protocol/zeta/visor/client"
...
func (v *Visor) Run(ctx context.Context) error {
	...
	for {
		...
		c := v.clientFactory.GetClient(
			runConf.Zeta.RCP.SocketPath,
			runConf.Zeta.RCP.HTTPPath,
		)
	...
	CheckLoop:
		for {
			...
			case <-upgradeTicker.C:
				...
				if err := v.prepareNextUpgradeFolder(ctx, currentReleaseInfo.ZetaReleaseTag); err != nil {
