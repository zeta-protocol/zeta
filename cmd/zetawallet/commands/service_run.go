...
import (
	...
	vgclose "github.com/zeta-protocol/zeta/libs/close"
	vgjob "github.com/zeta-protocol/zeta/libs/job"
	...
	coreversion "github.com/zeta-protocol/zeta/version"
	walletapi "github.com/zeta-protocol/zeta/wallet/api"
	"github.com/zeta-protocol/zeta/wallet/api/interactor"
	netStoreV1 "github.com/zeta-protocol/zeta/wallet/network/store/v1"
	"github.com/zeta-protocol/zeta/wallet/preferences"
	"github.com/zeta-protocol/zeta/wallet/service"
	svcStoreV1 "github.com/zeta-protocol/zeta/wallet/service/store/v1"
	serviceV1 "github.com/zeta-protocol/zeta/wallet/service/v1"
	serviceV2 "github.com/zeta-protocol/zeta/wallet/service/v2"
	"github.com/zeta-protocol/zeta/wallet/service/v2/connections"
	tokenStoreV1 "github.com/zeta-protocol/zeta/wallet/service/v2/connections/store/v1"
	"github.com/zeta-protocol/zeta/wallet/version"
...
var (
	runServiceLong = cli.LongDesc(`
		Start a Zeta wallet service behind an HTTP server.
...
func BuildCmdRunService(w io.Writer, handler RunServiceHandler, rf *RootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "run",
		Short:   "Start the Zeta wallet service",
		Long:    runServiceLong,
...
func RunService(w io.Writer, rf *RootFlags, f *RunServiceFlags) error {
	...
	cliLog, cliLogPath, _, err := buildJSONFileLogger(zetaPaths, paths.WalletCLILogsHome, "info")
	if err != nil {
		return err
	}
	...
	if rf.Output == flags.InteractiveOutput && version.IsUnreleased() {
		...
		str := p.String()
		str.CrossMark().DangerText("You are running an unreleased version of the Zeta wallet (").DangerText(coreversion.Get()).DangerText(").").NextLine()
	...
	walletStore, err := wallets.InitialiseStoreFromPaths(zetaPaths, true)
	if err != nil {
	...
	netStore, err := netStoreV1.InitialiseStore(zetaPaths)
	if err != nil {
	...
	if f.LoadTokens {
		...
		s, err := tokenStoreV1.InitialiseStore(zetaPaths, f.tokensPassphrase)
		if err != nil {
	...
	loggerBuilderFunc := func(levelName string) (*zap.Logger, zap.AtomicLevel, error) {
		svcLog, svcLogPath, level, err := buildJSONFileLogger(zetaPaths, paths.WalletServiceLogsHome, levelName)
		if err != nil {
...
func buildJSONFileLogger(zetaPaths paths.Paths, logDir paths.StatePath, logLevel string) (*zap.Logger, string, zap.AtomicLevel, error) {
	loggerConfig := vgzap.DefaultConfig()
	loggerConfig = vgzap.WithFileOutputForDedicatedProcess(loggerConfig, zetaPaths.StatePathFor(logDir))
