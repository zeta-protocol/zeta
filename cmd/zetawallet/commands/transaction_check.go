...
import (
	...
	walletnode "github.com/zeta-protocol/zeta/wallet/api/node"
	networkStore "github.com/zeta-protocol/zeta/wallet/network/store/v1"
...
var (
	checkTransactionLong = cli.LongDesc(`
		...
		The transaction should be a Zeta transaction formatted as a JSON payload, as follows:
	...
	checkTransactionExample = cli.Examples(`
		...
		# Check a transaction and send it to a specific Zeta node address
...
func NewCmdCheckTransaction(w io.Writer, rf *RootFlags) *cobra.Command {
	handler := func(params api.AdminCheckTransactionParams, passphrase string, log *zap.Logger) (api.AdminCheckTransactionResult, error) {
		...
		ns, err := networkStore.InitialiseStore(zetaPaths)
		if err != nil {
...
func BuildCmdCheckTransaction(w io.Writer, handler CheckTransactionHandler, rf *RootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "check",
		Short:   "Check a transaction and send it to a Zeta node",
	...
	cmd.Flags().StringVar(&f.NodeAddress,
		"node-address",
		"",
		"Zeta node address to which the transaction is sent after it is checked",
	)
