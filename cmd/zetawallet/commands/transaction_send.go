...
var (
	sendTransactionLong = cli.LongDesc(`
		Send a transaction to a Zeta node via the gRPC API. The transaction can be sent to
...
func BuildCmdSendTransaction(w io.Writer, handler SendTransactionHandler, rf *RootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "send",
		Short:   "Send a transaction to a Zeta node",
	...
	cmd.Flags().StringVar(&f.NodeAddress,
		"node-address",
		"",
		"Zeta node address to which the transaction is sent",
	)
