...
import (
	...
	"github.com/zeta-protocol/zeta/core/config/encoding"
...
type InitCmd struct {
	...
	Force bool `short:"f" long:"force" description:"Erase existing zeta configuration at the specified path"`
...
func (opts *InitCmd) Execute(args []string) error {
	...
	if mode == encoding.NodeModeValidator {
		...
		nwRegistry, err = registry.NewLoader(zetaPaths, pass)
		if err != nil {
			return err
		}
	}
...
func Init(ctx context.Context, parser *flags.Parser) error {
	...
	var (
		short = "Initializes a zeta node"
		long  = "Generate the minimal configuration required for a zeta node to start. You must specify 'full' or 'validator'"
	)
