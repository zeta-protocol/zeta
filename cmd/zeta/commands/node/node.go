...
import (
	...
	"github.com/zeta-protocol/zeta/core/api/rest"
	...
	"github.com/zeta-protocol/zeta/core/protocol"
...
func (n *Command) Run(
	...
	n.protocol, err = protocol.New(
		n.ctx, n.confWatcher, n.Log, n.cancel, n.stopBlockchain, n.nodeWallets, n.ethClient, n.ethConfirmations, n.blockchainClient, zetaPaths, n.stats)
	if err != nil {
		return err
	}
	...
	n.Log.Info("Zeta startup complete",
...
func (n *Command) Stop() error {
	...
	n.Log.Info("Zeta shutdown complete",
		logging.String("version", version.Get()),
...
func (n *Command) startAPIs() error {
	...
	if n.conf.IsValidator() {
		adminServer, err := admin.NewValidatorServer(n.Log, n.conf.Admin, n.zetaPaths, n.nodeWalletPassphrase, n.nodeWallets, n.protocol.GetProtocolUpgradeService())
		if err != nil {
			return err
		}
...
func (n *Command) setupCommon() (err error) {
	...
	if conf.Pprof.Enabled {
		n.Log.Info("zeta is starting with pprof profile, this is not a recommended setting for production")
...
func (n *Command) loadNodeWallets() (err error) {
	...
	n.nodeWallets, err = nodewallets.GetNodeWallets(n.conf.NodeWallet, n.zetaPaths, n.nodeWalletPassphrase)
	if err != nil {
