...
import (
	...
	vgfmt "github.com/zeta-protocol/zeta/libs/fmt"
...
type importCmd struct {
	...
	Chain      string              `short:"c" long:"chain" required:"true" description:"The chain to be imported" choice:"zeta" choice:"ethereum" choice:"tendermint"`
	...
	// clef flags
	EthereumClefAddress string `long:"ethereum-clef-address" description:"The URL to the clef instance that Zeta will use."`
	EthereumClefAccount string `long:"ethereum-clef-account" description:"The Ethereum account to be imported by Zeta from Clef. In hex."`
...
func (opts *importCmd) Execute(_ []string) error {
	...
	if opts.Chain == zetaChain || (opts.Chain == ethereumChain && opts.EthereumClefAddress == "") {
	...
	case zetaChain:
		data, err = nodewallets.ImportZetaWallet(zetaPaths, registryPass, walletPass, walletPath, opts.Force)
		if err != nil {
			return fmt.Errorf("couldn't import Zeta node wallet: %w", err)
		}
	case tendermintChain:
		...
		data, err = nodewallets.ImportTendermintPubkey(zetaPaths, registryPass, tendermintPubkey, opts.Force)
		if err != nil {
