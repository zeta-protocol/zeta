...
func (e *ERC20Cmd) GetSigner() (bridges.Signer, error) {
	zetaPaths := paths.New(e.ZetaHome)
	...
	if len(e.PrivateKey) <= 0 {
		...
		s, err = nodewallets.GetEthereumWallet(zetaPaths, pass)
		if err != nil {
...
func (opts *ERC20ListAssetCmd) Execute(_ []string) error {
	...
	bundle, err := erc20Logic.ListAsset(
		opts.TokenAddress, opts.ZetaAssetID, lifetimeLimit, withdrawThreshod, nonce,
	)
	if err != nil {
		return fmt.Errorf("unable to generate signature: %w", err)
	}
...
type ERC20SetBridgeAddressCmd struct {
	...
	AssetPoolAddress string `long:"asset-pool-address" required:"true" description:"The address of the zeta asset pool this transaction will be submitted to"`
	Nonce            string `long:"nonce" required:"true" description:"A nonce for this signature"`
}
