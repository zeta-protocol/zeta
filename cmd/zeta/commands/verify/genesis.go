...
func verifyValidators(r *reporter, validators map[string]validator) {
	...
	for tmkey, v := range validators {
		...
		if !isValidParty(v.ZetaPubKey) {
			r.Err("app_state.validators[%v] has an invalid zeta public key, `%v`", tmkey, v.ZetaPubKey)
		}

		if v.ZetaPubKeyIndex == 0 {
			r.Err("app_state.validators[%v] has an invalid zeta public key index, `%v`", tmkey, v.ZetaPubKeyIndex)
		}
