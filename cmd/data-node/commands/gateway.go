...
func (opts *gatewayCmd) Execute(_ []string) error {
	...
	cfgwatchr, err := config.NewWatcher(ctx, log, zetaPaths)
	if err != nil {
	...
	eg.Go(func() error {
		srv := server.New(opts.Config, log, zetaPaths)
...
func Gateway(ctx context.Context, parser *flags.Parser) error {
	...
	_, err := parser.AddCommand("gateway", "The API gateway", "The gateway for all the zeta APIs", opts)
	return err
}
