...
func (cmd *PostgresRunCmd) Execute(_ []string) error {
	...
	configWatcher, err := config.NewWatcher(ctx, log, zetaPaths, config.Use(parseFlagOpt))
	if err != nil {
		return err
	}
	...
	dbConfig := embeddedpostgres.DefaultConfig().
		...
		RuntimePath(zetaPaths.StatePathFor(paths.DataNodeStorageSQLStoreHome)).
		DataPath(zetaPaths.StatePathFor(paths.DataNodeStorageSQLStoreNodeDataHome))
