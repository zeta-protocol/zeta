...
func (cmd *loadCmd) Execute(args []string) error {
	...
	hasSchema, err := sqlstore.HasZetaSchema(ctx, connPool)
	if err != nil {
...
func createNetworkHistoryService(ctx context.Context, log *logging.Logger, zetaConfig config.Config, connPool *pgxpool.Pool, zetaPaths paths.Paths) (*networkhistory.Service, error) {
	snapshotService, err := snapshot.NewSnapshotService(log, zetaConfig.NetworkHistory.Snapshot, connPool,
		zetaPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), func(version int64) error {
			if err := sqlstore.MigrateUpToSchemaVersion(log, zetaConfig.SQLStore, version, sqlstore.EmbedMigrations); err != nil {
				return fmt.Errorf("failed to migrate to schema version %d: %w", version, err)
			}
			return nil
		},
		func(version int64) error {
			if err := sqlstore.MigrateDownToSchemaVersion(log, zetaConfig.SQLStore, version, sqlstore.EmbedMigrations); err != nil {
	...
	networkHistoryStore, err := store.New(ctx, log, zetaConfig.ChainID, zetaConfig.NetworkHistory.Store, zetaPaths.StatePathFor(paths.DataNodeNetworkHistoryHome),
		zetaConfig.MaxMemoryPercent)
	if err != nil {
		return nil, fmt.Errorf("failed to create network history store:%w", err)
	}

	networkHistoryService, err := networkhistory.NewWithStore(ctx, log, zetaConfig.ChainID, zetaConfig.NetworkHistory,
		connPool, snapshotService, networkHistoryStore, zetaConfig.API.Port,
		zetaPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo))
	if err != nil {
