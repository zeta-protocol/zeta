...
import (
	...
	"github.com/zeta-protocol/zeta/datanode/networkhistory/snapshot"
	...
	zetaprotoapi "github.com/zeta-protocol/zeta/protos/zeta/api/v1"
)
...
func (l *NodeCommand) persistentPre([]string) (err error) {
	...
	if conf.Pprof.Enabled {
		preLog.Info("zeta is starting with pprof profile, this is not a recommended setting for production")
	...
	if l.conf.SQLStore.UseEmbedded {
		...
		runtimeDir := l.zetaPaths.StatePathFor(paths.DataNodeEmbeddedPostgresRuntimeDir)
	...
	if l.conf.SQLStore.WipeOnStartup {
		if ResetDatabaseAndNetworkHistory(l.ctx, l.Log, l.zetaPaths, l.conf.SQLStore.ConnectionConfig); err != nil {
...
func (l *NodeCommand) initialiseDatabase(preLog *logging.Logger) error {
	...
	if !hasZetaSchema {
		err = sqlstore.RecreateZetaDatabase(l.ctx, preLog, l.conf.SQLStore.ConnectionConfig)
		if err != nil {
			return fmt.Errorf("failed to recreate zeta schema: %w", err)
		}
	}
...
func (l *NodeCommand) preRun([]string) (err error) {
	...
	if l.conf.Broker.UseBufferedEventSource {
		bufferFilePath, err := l.zetaPaths.CreateStatePathFor(paths.DataNodeEventBufferHome)
		if err != nil {
			preLog.Error("failed to create path for buffered event source", logging.Error(err))
			return err
		}

		archiveFilesPath, err := l.zetaPaths.CreateStatePathFor(paths.DataNodeArchivedEventBufferHome)
		if err != nil {
...
func (l *NodeCommand) initialiseNetworkHistory(preLog *logging.Logger, connConfig sqlstore.ConnectionConfig) error {
	...
	l.snapshotService, err = snapshot.NewSnapshotService(snapshotServiceLog, l.conf.NetworkHistory.Snapshot,
		networkHistoryPool,
		l.zetaPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), func(version int64) error {
	...
	l.networkHistoryService, err = networkhistory.New(l.ctx, networkHistoryServiceLog, l.conf.NetworkHistory,
		l.zetaPaths.StatePathFor(paths.DataNodeNetworkHistoryHome),
		networkHistoryPool,
		l.conf.ChainID, l.snapshotService, l.conf.API.Port, l.zetaPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyFrom),
		l.zetaPaths.StatePathFor(paths.DataNodeNetworkHistorySnapshotCopyTo), l.conf.MaxMemoryPercent)
	if err != nil {
...
func ResetDatabaseAndNetworkHistory(ctx context.Context, log *logging.Logger, zetaPaths paths.Paths, connConfig sqlstore.ConnectionConfig) error {
	err := os.RemoveAll(zetaPaths.StatePathFor(paths.DataNodeNetworkHistoryHome))
	if err != nil {
	...
	if err := sqlstore.RecreateZetaDatabase(ctx, log, connConfig); err != nil {
