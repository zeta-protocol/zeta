...
import (
	...
	"github.com/zeta-protocol/zeta/datanode/gateway/server"
...
func (l *NodeCommand) Run(ctx context.Context, cfgwatchr *config.Watcher, zetaPaths paths.Paths, args []string) error {
...
func (l *NodeCommand) runNode([]string) error {
	...
	if err := eg.Wait(); err != nil && !errors.Is(err, context.Canceled) {
		nodeLog.Error("Zeta data node stopped with error", logging.Error(err))
		return fmt.Errorf("zeta data node stopped with error: %w", err)
	}
