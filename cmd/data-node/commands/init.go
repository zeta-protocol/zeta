...
type InitCmd struct {
	...
	Force   bool `short:"f" long:"force" description:"Erase exiting zeta configuration at the specified path"`
...
func Init(ctx context.Context, parser *flags.Parser) error {
	...
	short := "init <chain ID>"
	long := "Generate the minimal configuration required for a zeta data-node to start. The Chain ID is required."
