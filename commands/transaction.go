...
import (
	...
	wcrypto "github.com/zeta-protocol/zeta/wallet/crypto"
...
func CheckTransaction(tx *commandspb.Transaction, chainID string) (*commandspb.InputData, error) {
	...
	} else if !IsZetaPubkey(tx.GetPubKey()) {
		errs.AddForProperty("tx.from.pub_key", ErrShouldBeAValidZetaPubkey)
	}
