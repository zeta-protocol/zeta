...
func checkUpdateAssetChanges(change *protoTypes.ProposalTerms_UpdateAsset) Errors {
	...
	} else if !IsZetaPubkey(change.UpdateAsset.AssetId) {
		errs.AddForProperty("proposal_submission.terms.change.update_asset.asset_id", ErrShouldBeAValidZetaID)
	}
...
func checkUpdateMarketChanges(change *protoTypes.ProposalTerms_UpdateMarket) Errors {
	...
	} else if !IsZetaPubkey(change.UpdateMarket.MarketId) {
		errs.AddForProperty("proposal_submission.terms.change.update_market.market_id", ErrShouldBeAValidZetaID)
	}
...
func checkDataSourceSpec(spec *zetapb.DataSourceDefinition, name string, parentProperty string, tryToSettle bool) Errors {
	errs := NewErrors()
	if spec == nil {
	...
	case *zetapb.DataSourceDefinition_External:
		...
		for i, key := range signers {
			...
			} else if pubkey := signer.GetSignerPubKey(); pubkey != nil && !crypto.IsValidZetaPubKey(pubkey.Key) {
				errs.AddForProperty(fmt.Sprintf("%s.%s.external.oracle.signers.%d", parentProperty, name, i), ErrIsNotValidZetaPubkey)
...
func isBindingMatchingSpec(spec *zetapb.DataSourceDefinition, bindingProperty string) bool {
