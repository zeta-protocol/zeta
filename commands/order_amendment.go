...
func checkOrderAmendment(cmd *commandspb.OrderAmendment) Errors {
	...
	} else if !IsZetaPubkey(cmd.OrderId) {
		errs.AddForProperty("order_amendment.order_id", ErrShouldBeAValidZetaID)
	}

	if len(cmd.MarketId) <= 0 {
		errs.AddForProperty("order_amendment.market_id", ErrIsRequired)
	} else if !IsZetaPubkey(cmd.MarketId) {
		errs.AddForProperty("order_amendment.market_id", ErrShouldBeAValidZetaID)
	}
