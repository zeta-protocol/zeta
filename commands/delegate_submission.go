...
func checkDelegateSubmission(cmd *commandspb.DelegateSubmission) Errors {
	...
	} else if !IsZetaPubkey(cmd.NodeId) {
		errs.AddForProperty("delegate_submission.node_id", ErrShouldBeAValidZetaPubkey)
	}
...
func checkUndelegateSubmission(cmd *commandspb.UndelegateSubmission) Errors {
	...
	if len(cmd.NodeId) <= 0 {
		...
		errs.AddForProperty("undelegate_submission.node_id", ErrShouldBeAValidZetaPubkey)
	}
